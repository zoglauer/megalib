...
MGUIEEntry::MGUIEEntry(const TGWindow* Parent, MString Label, bool Emphasize, 
                       long Value, bool Limits, long Min, long Max) :
  MGUIElement(Parent, kHorizontalFrame)
{
  // Creates a frame containing a label and an entry-box  
  //
...
void MGUIEEntry::Create()
{
  ...
  if (m_Limits == false) {
    ...
    if (m_Type == e_Integer) {
      m_NumberInput = new TGNumberEntry(this, m_ValueAsDouble, 10, -1, TGNumberFormat::kNESInteger);
    } else {
...
void MGUIEEntry::SetValue(long Value)
{
  m_ValueAsDouble = Value;
  m_Value = MakeSmartString(Value);
...
void MGUIEEntry::SetValue(unsigned long Value)
{
...
long MGUIEEntry::GetAsInt()
{
  ...
  if (m_Limits == false) {
    return (long) ToNumber(m_Input->GetText());  
  } else {
...
bool MGUIEEntry::IsInt(long Min, long Max)
{
