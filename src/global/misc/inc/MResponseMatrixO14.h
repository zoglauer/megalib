...
#ifndef __MResponseMatrixO14__
#define __MResponseMatrixO14__
...
class MResponseMatrixO14 : public MResponseMatrixOx
{
  // public interface:
 public:
  MResponseMatrixO14();
  MResponseMatrixO14(vector<float> x1Dim, vector<float> x2Dim, 
  ...
  MResponseMatrixO14(MString Name, 
                     ...
                     vector<float> x11Dim, vector<float> x12Dim, 
                     vector<float> x13Dim, vector<float> x14Dim);
  virtual ~MResponseMatrixO14();

  void Init();

  bool operator==(const MResponseMatrixO14& R);  
  MResponseMatrixO14& operator+=(const MResponseMatrixO14& R);  
  MResponseMatrixO14& operator-=(const MResponseMatrixO14& R);  
  MResponseMatrixO14& operator/=(const MResponseMatrixO14& R);  

  MResponseMatrixO14& operator+=(const float& Value);  
  MResponseMatrixO14& operator*=(const float& Value);  
  ...
  MString GetAxisName(unsigned int order = 14) const;
  ...
  void SetBinContent(unsigned int x1, unsigned int x2, unsigned int x3, 
                     ...
                     unsigned int x13, unsigned int x14, float Value = 1);
  ...
  void Add(float x1, float x2, float x3, float x4, 
           ...
           float x13, float x14, float Value = 1);
  void SetMatrix(unsigned int b, MResponseMatrixO13 R13);

  virtual unsigned long GetNBins() const;

  virtual float GetAxisContent(unsigned int b, unsigned int order = 14) const;
  virtual vector<float> GetAxis(unsigned int order = 14) const;
  virtual unsigned int GetAxisBins(unsigned int order = 14) const;
  virtual float GetAxisBinCenter(unsigned int b, unsigned int order = 14) const;
  virtual unsigned int GetAxisBin(float v, unsigned int order = 14) const;
  virtual float GetAxisMinimum(unsigned int order = 14) const;
  virtual float GetAxisMaximum(unsigned int order = 14) const;
  virtual float GetAxisLowEdge(unsigned int b, unsigned int order = 14) const;
  virtual float GetAxisHighEdge(unsigned int b, unsigned int order = 14) const;
  ...
  virtual float Get(float x1, float x2, float x3, float x4, 
                    float x5, float x6, float x7, float x8, 
                    float x9, float x10, float x11, float x12, 
                    float x13, float x14) const;
  virtual float GetInterpolated(float x1, float x2, float x3, float x4, 
                                float x5, float x6, float x7, float x8, 
                                float x9, float x10, float x11, float x12, 
                                float x13, float x14, bool DoExtrapolate = false) const;
  ...
  void Show(float x1, float x2, float x3, float x4, 
            float x5, float x6, float x7, float x8, 
            float x9, float x10, float x11, float x12, 
            float x13, float x14, bool Normalized = true);
 ...
 protected:
  ...
  // private members:
 private:
  MString m_NameAxisO14;
  vector<float> m_AxisO14;
  vector<MResponseMatrixO13> m_AxesO13;
  

  friend ostream& operator<<(ostream& os, const MResponseMatrixO14& R);


#ifdef ___CLING___
 public:
  ClassDef(MResponseMatrixO14, 1) // response matrix of order 14
#endif
