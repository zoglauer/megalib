...
#ifndef __MResponseMatrixO9__
#define __MResponseMatrixO9__
...
class MResponseMatrixO9 : public MResponseMatrixOx
{
  // public interface:
 public:
  MResponseMatrixO9();
  MResponseMatrixO9(vector<float> x1Dim, vector<float> x2Dim, 
                    vector<float> x3Dim, vector<float> x4Dim, 
                    vector<float> x5Dim, vector<float> x6Dim, 
                    vector<float> x7Dim, vector<float> x8Dim, 
                    vector<float> x9Dim);
  MResponseMatrixO9(MString Name, 
                    vector<float> x1Dim, vector<float> x2Dim, 
                    vector<float> x3Dim, vector<float> x4Dim, 
                    vector<float> x5Dim, vector<float> x6Dim, 
                    vector<float> x7Dim, vector<float> x8Dim, 
                    vector<float> x9Dim);
  virtual ~MResponseMatrixO9();

  void Init();

  bool operator==(const MResponseMatrixO9& R);  
  MResponseMatrixO9& operator+=(const MResponseMatrixO9& R);  
  MResponseMatrixO9& operator-=(const MResponseMatrixO9& R);  
  MResponseMatrixO9& operator/=(const MResponseMatrixO9& R);  

  MResponseMatrixO9& operator+=(const float& Value);  
  MResponseMatrixO9& operator*=(const float& Value);  
  ...
  void SetAxisNames(MString x1Name, MString x2Name, 
                    ...
                    MString x9Name);
  MString GetAxisName(unsigned int order = 9) const;
  ...
  virtual unsigned int GetAxisBins(unsigned int order = 9) const;
  virtual float GetAxisBinCenter(unsigned int b, unsigned int order = 9) const;
  virtual unsigned int GetAxisBin(float v, unsigned int order = 9) const;
  virtual float GetAxisMinimum(unsigned int order = 9) const;
  virtual float GetAxisMaximum(unsigned int order = 9) const;
  virtual float GetAxisLowEdge(unsigned int b, unsigned int order = 9) const;
  virtual float GetAxisHighEdge(unsigned int b, unsigned int order = 9) const;
  ...
  virtual float GetInterpolated(float x1, float x2, float x3, float x4, 
                                float x5, float x6, float x7, float x8, 
                                float x9, bool DoExtrapolate = false) const;
 ...
 protected:
  ...
  // private members:
 private:
  MString m_NameAxisO9;
  vector<float> m_AxisO9;
  vector<MResponseMatrixO8> m_AxesO8;
  

  friend ostream& operator<<(ostream& os, const MResponseMatrixO9& R);


#ifdef ___CLING___
 public:
  ClassDef(MResponseMatrixO9, 1) // response matrix of order 9
#endif
