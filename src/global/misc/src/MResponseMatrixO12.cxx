...
MResponseMatrixO12::MResponseMatrixO12() :
  MResponseMatrixOx()
{
  // default constructor
...
MResponseMatrixO12::MResponseMatrixO12(vector<float> x1Axis, vector<float> x2Axis, 
  ...
  MResponseMatrixOx()
{
  ...
  SetAxis(x1Axis, x2Axis, x3Axis, x4Axis, x5Axis, x6Axis, x7Axis, x8Axis, 
          x9Axis, x10Axis, x11Axis, x12Axis);
}
...
MResponseMatrixO12::MResponseMatrixO12(MString Name, 
...
bool MResponseMatrixO12::operator==(const MResponseMatrixO12& R)
{
  // Two matrixes are identical if they have the same axis:

  if (m_AxisO12 != R.m_AxisO12) {
    return false;
  }

  for (unsigned int i = 0; i < m_AxesO11.size(); ++i) {
    if (!(m_AxesO11[i] == R.m_AxesO11[i])) {
      return false;
    }
  }
...
MResponseMatrixO12& MResponseMatrixO12::operator+=(const MResponseMatrixO12& R)
{
  // Append a matrix to this one

  if (*this == R) {
    for (unsigned int i = 0; i < m_AxesO11.size(); ++i) {
      m_AxesO11[i] += R.m_AxesO11[i];
    }
  } else {
...
MResponseMatrixO12& MResponseMatrixO12::operator+=(const float& Value)
{
  // Append a matrix to this one
...
void MResponseMatrixO12::SetAxis(vector<float> x1Axis, vector<float> x2Axis, 
                                 vector<float> x3Axis, vector<float> x4Axis, 
                                 vector<float> x5Axis, vector<float> x6Axis, 
                                 vector<float> x7Axis, vector<float> x8Axis, 
                                 vector<float> x9Axis, vector<float> x10Axis, 
                                 vector<float> x11Axis, vector<float> x12Axis)
{
  ...
  if ((x1Axis.size()-1)*(x2Axis.size()-1)*(x3Axis.size()-1)*
      ...
      (x10Axis.size()-1)*(x11Axis.size()-1)*(x12Axis.size()-1) > c_SizeLimit) {
  ...
  m_AxesO11.resize(m_AxisO12.size()-1);
  for (unsigned int b = 0; b < m_AxisO12.size()-1; ++b) {
    m_AxesO11[b].SetAxis(x1Axis, x2Axis, x3Axis, x4Axis, x5Axis, x6Axis, x7Axis, x8Axis, x9Axis, x10Axis, x11Axis);
    m_AxesO11[b].SetName(m_Name);
  }
}
...
void MResponseMatrixO12::SetAxisNames(MString x1Name, MString x2Name, 
                                      ...
                                      MString x11Name, MString x12Name)
{
  ...
  m_NameAxisO12 = x12Name;
  for (unsigned int i = 0; i < m_AxesO11.size(); ++i) {
    m_AxesO11[i].SetAxisNames(x1Name, x2Name, x3Name, x4Name, x5Name, x6Name, x7Name, x8Name, x9Name, x10Name, x11Name);
  }
}
...
MString MResponseMatrixO12::GetAxisName(unsigned int order) const
{
  // Return the name of the axis

  massert(order >= 1 && order <= 12);

  if (order == 12) {
    return m_NameAxisO12;
  } else {
    massert(m_AxisO12.size() > 0);
    return m_AxesO11[0].GetAxisName(order);
  }
}
...
void MResponseMatrixO12::SetBinContent(unsigned int x1, unsigned int x2, 
                                       ...
                                       unsigned int x11, unsigned int x12, float Value)
{
  // Set the content of the bin

  massert(x12 < m_AxisO12.size()-1);
  
  m_AxesO11[x12].SetBinContent(x1, x2, x3, x4, x5, x6, x7, x8, x9, x10, x11, Value);
}
...
void MResponseMatrixO12::Add(float x1, float x2, float x3, float x4, 
                             float x5, float x6, float x7, float x8, 
                             float x9, float x10, float x11, float x12, float Value)
{
  /// Add a value to the bin closest to x, y, z

  // Get Position:
  int Position = -1; 
  for (unsigned int i = 0; i < m_AxisO12.size(); ++i) {
    if (m_AxisO12[i] > x12) {
      break;
    } 
    Position = (int) i;
  ...
  if (Position <= -1) {
    mdebug<<"Underflow in "<<m_Name
          <<": ("<<x1<<", "<<x2<<", "<<x3<<", "<<x4<<", "<<x5<<", "<<x6<<", "<<x7<<", "<<x8<<", "<<x9<<", "<<x10<<", "<<x11<<", "<<x12<<") = "<<Value<<endl;
    return;
  }
  if (Position >= (int) m_AxisO12.size()-1) {
    mdebug<<"Overflow in "
          <<m_Name<<": ("<<x1<<", "<<x2<<", "<<x3<<", "<<x4<<", "<<x5<<", "<<x6<<", "<<x7<<", "<<x8<<", "<<x9<<", "<<x10<<", "<<x11<<", "<<x12<<") = "<<Value<<endl;
    return;
  }

  m_AxesO11[Position].Add(x1, x2, x3, x4, x5, x6, x7, x8, x9, x10, x11, Value);
}
...
void MResponseMatrixO12::SetMatrix(unsigned int b, MResponseMatrixO11 R)
{
  // Set a whole sub matrix

  massert(b < m_AxesO11.size());
  m_AxesO11[b] = R;
}
...
unsigned long MResponseMatrixO12::GetNBins() const
{
  // Return the number of bins

  if (m_AxisO12.size() < 2) return 0;
  unsigned long Bins = m_AxisO12.size()-1;
  return Bins*m_AxesO11[0].GetNBins();
}
...
unsigned int MResponseMatrixO12::GetAxisBins(unsigned int order) const
{
  // Return the number of bins for the main axis O12
  ...
  if (order == 12) {
    ...
    return m_AxisO12.size()-1;
  } else {
    massert(m_AxisO12.size() > 0);
    return m_AxesO11[0].GetAxisBins(order);
  }
}
...
vector<float> MResponseMatrixO12::GetAxis(unsigned int order) const
{
  ...
  if (order == 12) {
    ...
    massert(GetNBins() > 0);
    return m_AxesO11[0].GetAxis(order);
  }
}
...
float MResponseMatrixO12::GetAxisContent(unsigned int b, unsigned int order) const
{
  // Return the axis content at bin b
  ...
  if (order == 12) {
    massert(b < m_AxisO12.size());
    return m_AxisO12[b];
  } else {
    massert(GetNBins() > 0);
    return m_AxesO11[0].GetAxisContent(b, order);
  }
}
...
float MResponseMatrixO12::GetAxisBinCenter(unsigned int b, unsigned int order) const
{
  ...
  if (order == 12) {
    massert(b < m_AxisO12.size()-1);
    return 0.5*(m_AxisO12[b]+m_AxisO12[b+1]);
  } else {
    massert(GetNBins() > 0);
    return m_AxesO11[0].GetAxisBinCenter(b, order);
  }
}
...
unsigned int MResponseMatrixO12::GetAxisBin(float x, unsigned int order) const
{
  // Return the center of axis bin b
  ...
  if (order == 12) {
    ...
    if (Position >= m_AxisO12.size()-1) {
      Position = c_Outside;
    }
    return Position;
  } else {
    massert(GetNBins() > 0);
    return m_AxesO11[0].GetAxisBin(x, order);
  }
}
...
float MResponseMatrixO12::GetAxisMinimum(unsigned int order) const
{
  ...
  if (order == 12) {
    ...
    massert(GetNBins() > 0);
    return m_AxesO11[0].GetAxisMinimum(order);
  }
}
...
float MResponseMatrixO12::GetAxisMaximum(unsigned int order) const
{
  ...
  if (order == 12) {
    massert(m_AxisO12.size() > 0);
    return m_AxisO12[m_AxisO12.size()-1];
  } else {
    massert(GetNBins() > 0);
    return m_AxesO11[0].GetAxisMaximum(order);
  }
}
...
float MResponseMatrixO12::GetAxisLowEdge(unsigned int b, unsigned int order) const
{
  ...
  if (order == 12) {
    ...
    massert(GetNBins() > 0);
    return m_AxesO11[0].GetAxisLowEdge(b, order);
  }
}
...
float MResponseMatrixO12::GetAxisHighEdge(unsigned int b, unsigned int order) const
{
  ...
  if (order == 12) {
    massert(b < m_AxisO12.size()-1);
    return m_AxisO12[b+1];
  } else {
    massert(GetNBins() > 0);
    return m_AxesO11[0].GetAxisHighEdge(b, order);
  } 
}
...
float MResponseMatrixO12::GetBinContent(unsigned int x1, unsigned int x2, 
                                        unsigned int x3, unsigned int x4, 
                                        unsigned int x5, unsigned int x6, 
                                        unsigned int x7, unsigned int x8, 
                                        unsigned int x9, unsigned int x10, 
                                        unsigned int x11, unsigned int x12) const
{
  ...
  return m_AxesO11[x12].GetBinContent(x1, x2, x3, x4, x5, x6, x7, x8, x9, x10, x11);
}
...
float MResponseMatrixO12::GetBinArea(unsigned int x1, unsigned int x2, 
                                     unsigned int x3, unsigned int x4, 
                                     unsigned int x5, unsigned int x6, 
                                     unsigned int x7, unsigned int x8, 
                                     unsigned int x9, unsigned int x10, 
                                     unsigned int x11, unsigned int x12) const
{
  ...
  return (m_AxisO12[x12+1]-m_AxisO12[x12])*m_AxesO11[x12].GetBinArea(x1, x2, x3, x4, x5, x6, x7, x8, x9, x10, x11);
}
...
float MResponseMatrixO12::GetBinContent(unsigned int x1, unsigned int x1axis, 
                                        ...
                                        unsigned int x12, unsigned int x12axis) const
{
  ...
  if (x1axis == 12) {
    return m_AxesO11[x1].GetBinContent(x2, x2axis, x3, x3axis, x4, x4axis, x5, x5axis, x6, x6axis, x7, x7axis, x8, x8axis, 
                                      x9, x9axis, x10, x10axis, x11, x11axis, x12, x12axis);
  } else if (x2axis == 12) {
    return m_AxesO11[x2].GetBinContent(x1, x1axis, x3, x3axis, x4, x4axis, x5, x5axis, x6, x6axis, x7, x7axis, x8, x8axis, 
                                      x9, x9axis, x10, x10axis, x11, x11axis, x12, x12axis);
  } else if (x3axis == 12) {
    return m_AxesO11[x3].GetBinContent(x1, x1axis, x2, x2axis, x4, x4axis, x5, x5axis, x6, x6axis, x7, x7axis, x8, x8axis, 
                                      x9, x9axis, x10, x10axis, x11, x11axis, x12, x12axis);
  } else if (x4axis == 12) {
    return m_AxesO11[x4].GetBinContent(x1, x1axis, x2, x2axis, x3, x3axis, x5, x5axis, x6, x6axis, x7, x7axis, x8, x8axis, 
                                      x9, x9axis, x10, x10axis, x11, x11axis, x12, x12axis);
  } else if (x5axis == 12) {
    return m_AxesO11[x5].GetBinContent(x1, x1axis, x2, x2axis, x3, x3axis, x4, x4axis, x6, x6axis, x7, x7axis, x8, x8axis, 
                                      x9, x9axis, x10, x10axis, x11, x11axis, x12, x12axis);
  } else if (x6axis == 12) {
    return m_AxesO11[x6].GetBinContent(x1, x1axis, x2, x2axis, x3, x3axis, x4, x4axis, x5, x5axis, x7, x7axis, x8, x8axis, 
                                      x9, x9axis, x10, x10axis, x11, x11axis, x12, x12axis);
  } else if (x7axis == 12) {
    return m_AxesO11[x7].GetBinContent(x1, x1axis, x2, x2axis, x3, x3axis, x4, x4axis, x5, x5axis, x6, x6axis, x8, x8axis, 
                                      x9, x9axis, x10, x10axis, x11, x11axis, x12, x12axis);
  } else if (x8axis == 12) {
    return m_AxesO11[x8].GetBinContent(x1, x1axis, x2, x2axis, x3, x3axis, x4, x4axis, x5, x5axis, x6, x6axis, x7, x7axis, 
                                      x9, x9axis, x10, x10axis, x11, x11axis, x12, x12axis);
  } else if (x9axis == 12) {
    return m_AxesO11[x9].GetBinContent(x1, x1axis, x2, x2axis, x3, x3axis, x4, x4axis, x5, x5axis, x6, x6axis, x7, x7axis, 
                                      x8, x8axis, x10, x10axis, x11, x11axis, x12, x12axis);
  } else if (x10axis == 12) {
    return m_AxesO11[x10].GetBinContent(x1, x1axis, x2, x2axis, x3, x3axis, x4, x4axis, x5, x5axis, x6, x6axis, x7, x7axis, 
                                       x8, x8axis, x9, x9axis, x11, x11axis, x12, x12axis);
  } else if (x11axis == 12) {
    return m_AxesO11[x11].GetBinContent(x1, x1axis, x2, x2axis, x3, x3axis, x4, x4axis, x5, x5axis, x6, x6axis, x7, x7axis, 
                                       x8, x8axis, x9, x9axis, x10, x10axis, x12, x12axis);
  } else if (x12axis == 12) {
    return m_AxesO11[x12].GetBinContent(x1, x1axis, x2, x2axis, x3, x3axis, x4, x4axis, x5, x5axis, x6, x6axis, x7, x7axis, 
                                       x8, x8axis, x9, x9axis, x10, x10axis, x11, x11axis);
  } else {
...
float MResponseMatrixO12::GetInterpolated(float x1, float x2, float x3, float x4, 
                                          float x5, float x6, float x7, float x8, 
                                          float x9, float x10, float x11, float x12, bool DoExtrapolate) const
{
  // Return the array-data according to value x1, x2, x3, x4, x5, x6, x7, x8, x9, x10, x11, x12
  ...
  if (m_ValuesCentered == true) { 
    if (m_AxisO12.size() == 2) {
      return m_AxesO11.front().GetInterpolated(x1, x2, x3, x4, x5, x6, x7, x8, x9, x10, x11, DoExtrapolate);
    } else {
      // Get Position:
      int Position = FindBinCentered(m_AxisO12, x12);
      ...
      } else if (Position >= int(m_AxesO11.size()-1)) {
        if (DoExtrapolate == true) {
          Position = int(m_AxesO11.size()-2); // extrapolate above higher edge
          // limits of highest CENTERED bin are m_AxesO11.size()-2 and m_AxesO11.size()-1 !!
        } else {
          return m_AxesO11.back().GetInterpolated(x1, x2, x3, x4, x5, x6, x7, x8, x9, x10, x11, DoExtrapolate);
        }
      }

      // Interpolate:
      return m_AxesO11[Position].GetInterpolated(x1, x2, x3, x4, x5, x6, x7, x8, x9, x10, x11, DoExtrapolate) + (x12 - GetAxisBinCenter(Position))/
        (GetAxisBinCenter(Position+1) - GetAxisBinCenter(Position))*
        (m_AxesO11[Position+1].GetInterpolated(x1, x2, x3, x4, x5, x6, x7, x8, x9, x10, x11, DoExtrapolate) - 
         m_AxesO11[Position].GetInterpolated(x1, x2, x3, x4, x5, x6, x7, x8, x9, x10, x11, DoExtrapolate));
    }
  } else {
    // Get Position:
    int Position = FindBin(m_AxisO12, x12);
    ...
    } else if (Position >= int(m_AxisO12.size()-1)) {
      if (DoExtrapolate == true) {
        Position = int(m_AxisO12.size()-2); // extrapolate above higher edge
        // limits of highest bin are m_AxisO12.size()-2 and  m_AxisO12.size()-1 !!
      } else {
        return m_AxesO11.back().GetInterpolated(x1, x2, x3, x4, x5, x6, x7, x8, x9, x10, x11, DoExtrapolate);
      }
    }
    ...
    // Interpolate:
    return m_AxesO11[Position].GetInterpolated(x1, x2, x3, x4, x5, x6, x7, x8, x9, x10, x11, DoExtrapolate) + (x12 - m_AxisO12[Position])/
      (m_AxisO12[Position+1] - m_AxisO12[Position])*
      (m_AxesO11[Position+1].GetInterpolated(x1, x2, x3, x4, x5, x6, x7, x8, x9, x10, x11, DoExtrapolate) - 
       m_AxesO11[Position].GetInterpolated(x1, x2, x3, x4, x5, x6, x7, x8, x9, x10, x11, DoExtrapolate));
  }
}
...
float MResponseMatrixO12::Get(float x1, float x2, float x3, float x4, float x5, 
                              float x6, float x7, float x8, float x9, float x10, 
                              float x11, float x12) const
{
  ...
  return m_AxesO11[Position].Get(x1, x2, x3, x4, x5, x6, x7, x8, x9, x10, x11);
}
...
float MResponseMatrixO12::GetMaximum() const
{
  // Return the maximum
...
float MResponseMatrixO12::GetMinimum() const
{
  // Return the minimum
...
double MResponseMatrixO12::GetSum() const
{
  // Return the sum of all bins:

  double Sum = 0;
  for (unsigned int i = 0; i < m_AxesO11.size(); ++i) {
    Sum += m_AxesO11[i].GetSum();
  }  
...
MResponseMatrixO1 MResponseMatrixO12::GetSumMatrixO1(unsigned int a1) const
{
  // Return the sum of the matrix as function of dimension order
  // Attention: Very inefficient algorithm!

  massert(AreIncreasing(12, a1));
  ...
  if (a1 == 12) {
    for (unsigned int i = 0; i < m_AxesO11.size(); ++i) {
      R.SetBinContent(i, m_AxesO11[i].GetSum());
    }
  } else {
    for (unsigned int i = 0; i < m_AxesO11.size(); ++i) {
      R += m_AxesO11[i].GetSumMatrixO1(a1);
    }
  }
...
MResponseMatrixO2 MResponseMatrixO12::GetSumMatrixO2(unsigned int a1, 
                                                    unsigned int a2) const
{
  // Return the sum of the matrix as function of dimension order
  // Attention: Simple, but very inefficient algorithm!

  massert(AreIncreasing(12, a1, a2));
  ...
  if (a2 == 12) {
    for (unsigned int i = 0; i < m_AxesO11.size(); ++i) {
      R.SetMatrix(i, m_AxesO11[i].GetSumMatrixO1(a1));
    }
  } else {
    for (unsigned int i = 0; i < m_AxesO11.size(); ++i) {
      R += m_AxesO11[i].GetSumMatrixO2(a1, a2);
    }
  }
...
MResponseMatrixO3 MResponseMatrixO12::GetSumMatrixO3(unsigned int a1, 
                                                    unsigned int a2,
                                                    unsigned int a3) const
{
  ...
  massert(AreIncreasing(12, a1, a2, a3));
  ...
  if (a3 == 12) {
    for (unsigned int i = 0; i < m_AxesO11.size(); ++i) {
      R.SetMatrix(i, m_AxesO11[i].GetSumMatrixO2(a1, a2));
    }
  } else {
    for (unsigned int i = 0; i < m_AxesO11.size(); ++i) {
      R += m_AxesO11[i].GetSumMatrixO3(a1, a2, a3);
    }
  }
...
MResponseMatrixO4 MResponseMatrixO12::GetSumMatrixO4(unsigned int a1, 
                                                    unsigned int a2,
                                                    unsigned int a3,
                                                    unsigned int a4) const
{
  ...
  massert(AreIncreasing(12, a1, a2, a3, a4));
  ...
  if (a4 == 12) {
    for (unsigned int i = 0; i < m_AxesO11.size(); ++i) {
      R.SetMatrix(i, m_AxesO11[i].GetSumMatrixO3(a1, a2, a3));
    }
  } else {
    for (unsigned int i = 0; i < m_AxesO11.size(); ++i) {
      R += m_AxesO11[i].GetSumMatrixO4(a1, a2, a3, a4);
    }
  }
...
MResponseMatrixO5 MResponseMatrixO12::GetSumMatrixO5(unsigned int a1, 
                                                    unsigned int a2,
                                                    unsigned int a3,
                                                    unsigned int a4,
                                                    unsigned int a5) const
{
  ...
  massert(AreIncreasing(12, a1, a2, a3, a4, a5));
  ...
  if (a5 == 12) {
    for (unsigned int i = 0; i < m_AxesO11.size(); ++i) {
      R.SetMatrix(i, m_AxesO11[i].GetSumMatrixO4(a1, a2, a3, a4));
    }
  } else {
    for (unsigned int i = 0; i < m_AxesO11.size(); ++i) {
      R += m_AxesO11[i].GetSumMatrixO5(a1, a2, a3, a4, a5);
    }
  }
...
MResponseMatrixO6 MResponseMatrixO12::GetSumMatrixO6(unsigned int a1, 
                                                    unsigned int a2,
                                                    unsigned int a3,
                                                    unsigned int a4,
                                                    unsigned int a5,
                                                    unsigned int a6) const
{
  ...
  massert(AreIncreasing(12, a1, a2, a3, a4, a5, a6));
  ...
  if (a6 == 12) {
    for (unsigned int i = 0; i < m_AxesO11.size(); ++i) {
      R.SetMatrix(i, m_AxesO11[i].GetSumMatrixO5(a1, a2, a3, a4, a5));
    }
  } else {
    for (unsigned int i = 0; i < m_AxesO11.size(); ++i) {
      R += m_AxesO11[i].GetSumMatrixO6(a1, a2, a3, a4, a5, a6);
    }
  }
...
MResponseMatrixO7 MResponseMatrixO12::GetSumMatrixO7(unsigned int a1, 
                                                    unsigned int a2,
                                                    unsigned int a3,
                                                    unsigned int a4,
                                                    unsigned int a5,
                                                    ...
                                                    unsigned int a7) const
{
  ...
  massert(AreIncreasing(12, a1, a2, a3, a4, a5, a6, a7));
  ...
  if (a7 == 12) {
    for (unsigned int i = 0; i < m_AxesO11.size(); ++i) {
      R.SetMatrix(i, m_AxesO11[i].GetSumMatrixO6(a1, a2, a3, a4, a5, a6));
    }
  } else {
    for (unsigned int i = 0; i < m_AxesO11.size(); ++i) {
      R += m_AxesO11[i].GetSumMatrixO7(a1, a2, a3, a4, a5, a6, a7);
    }
  }
...
MResponseMatrixO8 MResponseMatrixO12::GetSumMatrixO8(unsigned int a1, 
                                                     unsigned int a2,
                                                     unsigned int a3,
                                                     unsigned int a4,
                                                     unsigned int a5,
                                                     ...
                                                     unsigned int a8) const
{
  ...
  massert(AreIncreasing(12, a1, a2, a3, a4, a5, a6, a7, a8));
  ...
  if (a8 == 12) {
    for (unsigned int i = 0; i < m_AxesO11.size(); ++i) {
      R.SetMatrix(i, m_AxesO11[i].GetSumMatrixO7(a1, a2, a3, a4, a5, a6, a7));
    }
  } else {
    for (unsigned int i = 0; i < m_AxesO11.size(); ++i) {
      R += m_AxesO11[i].GetSumMatrixO8(a1, a2, a3, a4, a5, a6, a7, a8);
    }
  }
...
MResponseMatrixO9 MResponseMatrixO12::GetSumMatrixO9(unsigned int a1, 
                                                     unsigned int a2,
                                                     unsigned int a3,
                                                     unsigned int a4,
                                                     unsigned int a5,
                                                     ...
                                                     unsigned int a9) const
{
  ...
  massert(AreIncreasing(12, a1, a2, a3, a4, a5, a6, a7, a8, a9));
  ...
  if (a9 == 12) {
    for (unsigned int i = 0; i < m_AxesO11.size(); ++i) {
      R.SetMatrix(i, m_AxesO11[i].GetSumMatrixO8(a1, a2, a3, a4, a5, a6, a7, a8));
    }
  } else {
    for (unsigned int i = 0; i < m_AxesO11.size(); ++i) {
      R += m_AxesO11[i].GetSumMatrixO9(a1, a2, a3, a4, a5, a6, a7, a8, a9);
    }
  }
...
MResponseMatrixO10 MResponseMatrixO12::GetSumMatrixO10(unsigned int a1, 
                                                       unsigned int a2,
                                                       unsigned int a3,
                                                       unsigned int a4,
                                                       unsigned int a5,
                                                       ...
                                                       unsigned int a10) const
{
  ...
  massert(AreIncreasing(12, a1, a2, a3, a4, a5, a6, a7, a8, a9, a10));
  ...
  if (a10 == 12) {
    for (unsigned int i = 0; i < m_AxesO11.size(); ++i) {
      R.SetMatrix(i, m_AxesO11[i].GetSumMatrixO9(a1, a2, a3, a4, a5, a6, a7, a8, a9));
    }
  } else {
    for (unsigned int i = 0; i < m_AxesO11.size(); ++i) {
      R += m_AxesO11[i].GetSumMatrixO10(a1, a2, a3, a4, a5, a6, a7, a8, a9, a10);
    }
  }
...
MResponseMatrixO11 MResponseMatrixO12::GetSumMatrixO11(unsigned int a1, 
                                                       unsigned int a2,
                                                       unsigned int a3,
                                                       unsigned int a4,
                                                       unsigned int a5,
                                                       ...
                                                       unsigned int a11) const
{
  ...
  massert(AreIncreasing(12, a1, a2, a3, a4, a5, a6, a7, a8, a9, a10, a11));
  ...
  if (a11 == 12) {
    for (unsigned int i = 0; i < m_AxesO11.size(); ++i) {
      R.SetMatrix(i, m_AxesO11[i].GetSumMatrixO10(a1, a2, a3, a4, a5, a6, a7, a8, a9, a10));
    }
  } else {
    for (unsigned int i = 0; i < m_AxesO11.size(); ++i) {
      R += m_AxesO11[i].GetSumMatrixO11(a1, a2, a3, a4, a5, a6, a7, a8, a9, a10, a11);
    }
  }
...
MResponseMatrixO12 MResponseMatrixO12::GetSumMatrixO12(unsigned int a1, 
                                                       unsigned int a2, 
                                                       unsigned int a3, 
                                                       unsigned int a4, 
                                                       unsigned int a5, 
                                                       ...
                                                       unsigned int a12) const
{
  ...
  massert(AreIncreasing(12, a1, a2, a3, a4, a5, a6, a7, a8, a9, a10, a11, a12));
...
bool MResponseMatrixO12::ReadSpecific(MFileResponse& Parser, 
                                     const MString& Type, 
                                     const int Version)
{
  ...
  if (Type == "ResponseMatrixO12") {
    merr<<"MResponseMatrixO12: Non-stream reading is not supported!"<<endl;
    /*
    while (Parser.TokenizeLine(T, true) == true) {
      ...
      if (T.GetTokenAt(0) == "R2") {
        if (T.GetNTokens() == 4) {
          ...
          mout<<"MResponseMatrixO12: Wrong number of arguments for token R2!"<<endl;
          Ok = false;
          break;
        }
      }
    ...
    */
    Ok = false;
  } else if (Type == "ResponseMatrixO12Stream") {
    vector<float> x1Axis;
    vector<float> x2Axis;
    vector<float> x3Axis;
    vector<float> x4Axis;
    ...
    while (Parser.TokenizeLine(T, true) == true) {
      ...
      if (T.GetTokenAt(0) == "A1") {
        ...
        if ((x1Axis.size()-1)*(x2Axis.size()-1)*(x3Axis.size()-1)*
            (x4Axis.size()-1)*(x5Axis.size()-1)*(x6Axis.size()-1)*
            (x7Axis.size()-1)*(x8Axis.size()-1)*(x9Axis.size()-1)*
            (x10Axis.size()-1)*(x11Axis.size()-1)*(x12Axis.size()-1) != StreamSize) {
          mout<<"MResponseMatrixO12: Axis sizes and stream size are not in sync!"<<endl;
          Ok = false;
          break;
        }
        SetAxis(x1Axis, x2Axis, x3Axis, x4Axis, x5Axis, x6Axis, x7Axis, x8Axis, x9Axis, x10Axis, x11Axis, x12Axis);
        SetAxisNames(x1Name, x2Name, x3Name, x4Name, x5Name, x6Name, x7Name, x8Name, x9Name, x10Name, x11Name, x12Name);
                          ...
                          for (x3 = 0; x3 < x3_max; ++x3) {
                            for (x2 = 0; x2 < x2_max; ++x2) {
                              for (x1 = 0; x1 < x1_max; ++x1) {
                                if (StreamSize == 0) {
                                  mout<<"MResponseMatrixO12: Stream has not the correct length (too small)!"<<endl;
                                  StreamOk = false;
                                  break;
                                }
                                if (Parser.GetFloat(Data) == true) {
                                  SetBinContent(x1, x2, x3, x4, x5, x6, x7, x8, x9, x10, x11, x12, Data);
                                  StreamSize--;
                                } else {
                                  mout<<"MResponseMatrixO12: Cannot parse a float!"<<endl;
                                  StreamOk = false;
                                  break;
                                }
                              }
                              ...
                              if (StreamOk == false) {
                                mout<<"MResponseMatrixO12: Stream was not ok!"<<endl;
                                Ok = false;
                                break;
                              }
                            }
    ...
    mout<<"MResponseMatrixO12: Wrong file type: "<<Type<<endl;
    Ok = false;    
  }
...
bool MResponseMatrixO12::Write(MString FileName, bool Stream)
{
  ...
  if (Stream == false) {
    merr<<"MResponseMatrixO12: Non-stream writing is not supported!"<<endl;
    /*
    s<<"Type ResponseMatrixO12"<<endl;
    for (x12 = 0; x12 < x12_max; ++x12) {
      for (x11 = 0; x11 < x11_max; ++x11) {
        for (x10 = 0; x10 < x10_max; ++x10) {
          for (x9 = 0; x9 < x9_max; ++x9) {
                        ...
                        for (x2 = 0; x2 < x2_max; ++x2) {
                          for (x1 = 0; x1 < x1_max; ++x1) {
                            s<<"R12 "
                              ...
                              <<" "<<GetBinContent(x1, x2, x3, x4, x5, x6, x7, x8, x9, x10, x11, x12)<<endl;
                            File.Write(s);
                          }
                        }
                      }
    ...
    */
    ...
    s<<"Type ResponseMatrixO12Stream"<<endl;
    // Write x1-axis
    s<<"A1 ";
    File.Write(s);
    for (x1 = 0; x1 <= x1_max; ++x1) {
                  ...
                  for (x5 = 0; x5 < x5_max; ++x5) {
                    for (x4 = 0; x4 < x4_max; ++x4) {
                      for (x3 = 0; x3 < x3_max; ++x3) {
                        for (x2 = 0; x2 < x2_max; ++x2) {
                          for (x1 = 0; x1 < x1_max; ++x1) {
                            File.Write(GetBinContent(x1, x2, x3, x4, x5, x6, x7, x8, x9, x10, x11, x12));
                          }
                        }
                      }
                    }
  ...
  mdebug<<"File \""<<FileName<<"\" with "
        <<x1_max*x2_max*x3_max*x4_max*x5_max*x6_max*x7_max*x8_max*x9_max*x10_max*x11_max*x12_max
...
void MResponseMatrixO12::Show(float x1, float x2, float x3, float x4, 
                              float x5, float x6, float x7, float x8, 
                              float x9, float x10, float x11, 
                              float x12, bool Normalize)
{
  // Create a 3d ROOT histogram:
  
  vector<unsigned int> axes;
  vector<unsigned int> values;
  vector<float> type(12, MResponseMatrix::c_ShowNo);
  vector<bool> found(12, false);
  ...
  mout<<"Axis mapping: "<<endl;
  for (unsigned int i = 0; i < axes.size(); ++i) {
    mout<<i<<": internal axis = "<<axes[i];
    if (type[i] == c_ShowNo) {
      mout<<" - axis bin = "<<values[i];
    }
    mout<<endl;
  }
  ...
  for (unsigned int i = 0; i < values.size(); ++i) {
    if (values[i] == c_Outside && type[i] == c_ShowNo) {
      mout<<"Error: Axis "<<axes[i]<<" ("<<GetAxisName(axes[i])<<") out of bounds!"<<endl;
      OutOfBounds = true;
    }
  }
  ...
  if (GetNBins() > 0) {
    ...
    if (NAxes == 1) {
      ...
      Hist = new TH1D(m_Name + "_RM12", m_Name + "_RM12", GetAxisBins(axes[0]), Bins);
      Hist->SetStats(true);
      Hist->SetContour(50);
      Hist->SetXTitle(GetAxisName(axes[0]));
      delete [] Bins;
    
      mout<<"Response matrix of order 12 with "<<GetNBins()<<" entries:"<<endl;
      double Norm = 1;
      for (unsigned int i1 = 0; i1 < GetAxisBins(axes[0]); ++i1) {
        ...
        Hist->SetBinContent(i1+1, GetBinContent(i1, axes[0], 
                                                ...
                                                values[9], axes[9], 
                                                values[10], axes[10], 
                                                values[11], axes[11])*Norm);
      }
    
      TCanvas* Canvas = new TCanvas(m_Name + "_RM12C", m_Name + "_RM12C", 0, 0, 600, 600);
      Canvas->cd();
      Hist->Draw();
      Canvas->Update();
      ...
      Hist = new TH2D(m_Name+"_RM12", m_Name+"_RM12", GetAxisBins(axes[0]), xBins, GetAxisBins(axes[1]), yBins);
      Hist->SetStats(true);
      Hist->SetContour(50);
      Hist->SetXTitle(GetAxisName(axes[0]));
      ...
      Hist = new TH3D(m_Name+"_RM12", m_Name+"_RM12", 
    ...
    mout<<"Empty response matrix of order 12"<<endl;
  }
}
...
{
  if (R.GetNBins() > 0) {
    os<<"Response matrix of order 12 with "<<R.GetNBins()<<" entries:"<<endl;
                      ...
                      for (unsigned int i3 = 0; i3 < R.GetAxisBins(3); ++i3) {
                        for (unsigned int i2 = 0; i2 < R.GetAxisBins(2); ++i2) {
                          for (unsigned int i1 = 0; i1 < R.GetAxisBins(1); ++i1) {
                            os<<"x1/x2/x3/x4/x5/x6/x7/x8/x9/x10/x11/x12: "
                            <<i1<<"/"<<i2<<"/"<<i3<<"/"<<i4<<"/"<<i5<<"/"<<i6<<"/"<<i7<<"/"<<i8<<"/"<<i9<<"/"<<i10<<"/"<<i11<<"/"<<i12
                              ...
                              <<"  - value : "<<R.GetBinContent(i1, i2, i3, i4, i5, i6, i7, i8, i9, i10, i11, i12)<<endl;
                          }
                        }
                      }
                    }
    ...
    os<<"Empty response matrix of order 12"<<endl;
  }
  return os;
}


// MResponseMatrixO12.cxx: the end...
////////////////////////////////////////////////////////////////////////////////
