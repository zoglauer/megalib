...
MResponseMatrixO9::MResponseMatrixO9(vector<float> x1Axis, vector<float> x2Axis, 
  ...
  MResponseMatrix()
{
  ...
  Init();
  SetAxis(x1Axis, x2Axis, x3Axis, x4Axis, x5Axis, x6Axis, x7Axis, x8Axis, x9Axis);
}
...
MResponseMatrixO9::MResponseMatrixO9(MString Name, vector<float> x1Axis, 
...
bool MResponseMatrixO9::operator==(const MResponseMatrixO9& R)
{
  // Two matrixes are identical if they have the same axis:

  if (m_AxisO9 != R.m_AxisO9) {
    return false;
  }

  for (unsigned int i = 0; i < m_AxesO8.size(); ++i) {
    if (!(m_AxesO8[i] == R.m_AxesO8[i])) {
      return false;
    }
  }
...
MResponseMatrixO9& MResponseMatrixO9::operator+=(const MResponseMatrixO9& R)
{
  // Append a matrix to this one

  if (*this == R) {
    for (unsigned int i = 0; i < m_AxesO8.size(); ++i) {
      m_AxesO8[i] += R.m_AxesO8[i];
    }
  } else {
...
MResponseMatrixO9& MResponseMatrixO9::operator+=(const float& Value)
{
  // Append a matrix to this one
...
void MResponseMatrixO9::SetAxis(vector<float> x1Axis, vector<float> x2Axis, 
                                vector<float> x3Axis, vector<float> x4Axis, 
                                vector<float> x5Axis, vector<float> x6Axis, 
                                vector<float> x7Axis, vector<float> x8Axis, 
                                vector<float> x9Axis)
{
  ...
  if ((x1Axis.size()-1)*(x2Axis.size()-1)*(x3Axis.size()-1)*
      ...
      (x7Axis.size()-1)*(x8Axis.size()-1)*(x9Axis.size()-1) > c_SizeLimit) {
  ...
  m_AxesO8.resize(m_AxisO9.size()-1);
  for (unsigned int b = 0; b < m_AxisO9.size()-1; ++b) {
    m_AxesO8[b].SetAxis(x1Axis, x2Axis, x3Axis, x4Axis, x5Axis, x6Axis, x7Axis, x8Axis);
    m_AxesO8[b].SetName(m_Name);
  }
}
...
void MResponseMatrixO9::SetAxisNames(MString x1Name, MString x2Name, 
                                     ...
                                     MString x9Name)
{
  ...
  m_NameAxisO9 = x9Name;
  for (unsigned int i = 0; i < m_AxesO8.size(); ++i) {
    m_AxesO8[i].SetAxisNames(x1Name, x2Name, x3Name, x4Name, x5Name, x6Name, x7Name, x8Name);
  }
}
...
MString MResponseMatrixO9::GetAxisName(unsigned int order) const
{
  ...
  if (order == 9) {
    ...
    return m_AxesO8[0].GetAxisName(order);
  }
}
...
void MResponseMatrixO9::SetBinContent(unsigned int x1, unsigned int x2, 
                                      unsigned int x3, unsigned int x4, 
                                      unsigned int x5, unsigned int x6, 
                                      unsigned int x7, unsigned int x8, 
                                      unsigned int x9, float Value)
{
  // Set the content of the bin

  massert(x9 < m_AxisO9.size()-1);
  
  m_AxesO8[x9].SetBinContent(x1, x2, x3, x4, x5, x6, x7, x8, Value);
}
...
void MResponseMatrixO9::Add(float x1, float x2, float x3, float x4, 
                            float x5, float x6, float x7, float x8, 
                            float x9, float Value)
{
  /// Add a value to the bin closest to x, y, z

  // Get Position:
  int Position = -1; 
  for (unsigned int i = 0; i < m_AxisO9.size(); ++i) {
    if (m_AxisO9[i] > x9) {
      break;
    } 
    Position = (int) i;
  ...
  if (Position <= -1) {
    mdebug<<"Underflow in "<<m_Name
          <<": ("<<x1<<", "<<x2<<", "<<x3<<", "<<x4<<", "<<x5<<", "<<x6<<", "<<x7<<", "<<x8<<", "<<x9<<") = "<<Value<<endl;
    return;
  }
  if (Position >= (int) m_AxisO9.size()-1) {
    mdebug<<"Overflow in "
          <<m_Name<<": ("<<x1<<", "<<x2<<", "<<x3<<", "<<x4<<", "<<x5<<", "<<x6<<", "<<x7<<", "<<x8<<", "<<x9<<") = "<<Value<<endl;
    return;
  }

  m_AxesO8[Position].Add(x1, x2, x3, x4, x5, x6, x7, x8, Value);
}
...
void MResponseMatrixO9::SetMatrix(unsigned int b, MResponseMatrixO8 R)
{
...
unsigned int MResponseMatrixO9::GetAxisBins(unsigned int order) const
{
  // Return the number of bins for the main axis O9
  ...
  if (order == 9) {
    ...
    massert(m_AxisO9.size() > 0);
    return m_AxesO8[0].GetAxisBins(order);
  }
}
...
vector<float> MResponseMatrixO9::GetAxis(unsigned int order) const
{
  ...
  if (order == 9) {
    ...
    massert(GetNBins() > 0);
    return m_AxesO8[0].GetAxis(order);
  }
}
...
float MResponseMatrixO9::GetAxisContent(unsigned int b, unsigned int order) const
{
  // Return the axis content at bin b
  ...
  if (order == 9) {
    ...
    massert(GetNBins() > 0);
    return m_AxesO8[0].GetAxisContent(b, order);
  }
}
...
float MResponseMatrixO9::GetAxisBinCenter(unsigned int b, unsigned int order) const
{
  ...
  if (order == 9) {
    massert(b < m_AxisO9.size()-1);
    return 0.5*(m_AxisO9[b]+m_AxisO9[b+1]);
  } else {
    massert(GetNBins() > 0);
    return m_AxesO8[0].GetAxisBinCenter(b, order);
  }
}
...
unsigned int MResponseMatrixO9::GetAxisBin(float x, unsigned int order) const
{
  // Return the center of axis bin b
  ...
  if (order == 9) {
    ...
    if (Position >= m_AxisO9.size()-1) {
      Position = c_Outside;
    }
    return Position;
  } else {
    massert(GetNBins() > 0);
    return m_AxesO8[0].GetAxisBin(x, order);
  }
}
...
float MResponseMatrixO9::GetAxisMinimum(unsigned int order) const
{
  ...
  if (order == 9) {
    ...
    massert(GetNBins() > 0);
    return m_AxesO8[0].GetAxisMinimum(order);
  }
}
...
float MResponseMatrixO9::GetAxisMaximum(unsigned int order) const
{
  ...
  if (order == 9) {
    massert(m_AxisO9.size() > 0);
    return m_AxisO9[m_AxisO9.size()-1];
  } else {
    massert(GetNBins() > 0);
    return m_AxesO8[0].GetAxisMaximum(order);
  }
}
...
float MResponseMatrixO9::GetAxisLowEdge(unsigned int b, unsigned int order) const
{
  ...
  if (order == 9) {
    ...
    massert(GetNBins() > 0);
    return m_AxesO8[0].GetAxisLowEdge(b, order);
  }
}
...
float MResponseMatrixO9::GetAxisHighEdge(unsigned int b, unsigned int order) const
{
  ...
  if (order == 9) {
    ...
    massert(GetNBins() > 0);
    return m_AxesO8[0].GetAxisHighEdge(b, order);
  } 
}
...
float MResponseMatrixO9::GetBinContent(unsigned int x1, unsigned int x2, 
                                       unsigned int x3, unsigned int x4, 
                                       unsigned int x5, unsigned int x6, 
                                       unsigned int x7, unsigned int x8, 
                                       unsigned int x9) const
{
  ...
  return m_AxesO8[x9].GetBinContent(x1, x2, x3, x4, x5, x6, x7, x8);
}
...
float MResponseMatrixO9::GetBinArea(unsigned int x1, unsigned int x2, 
                                    unsigned int x3, unsigned int x4, 
                                    unsigned int x5, unsigned int x6, 
                                    unsigned int x7, unsigned int x8, 
                                    unsigned int x9) const
{
  ...
  return (m_AxisO9[x9+1]-m_AxisO9[x9])*m_AxesO8[x9].GetBinArea(x1, x2, x3, x4, x5, x6, x7, x8);
}
...
float MResponseMatrixO9::GetBinContent(unsigned int x1, unsigned int x1axis, 
                                       ...
                                       unsigned int x9, unsigned int x9axis) const
{
  // 
  ...
  if (x1axis == 9) {
    return m_AxesO8[x1].GetBinContent(x2, x2axis, x3, x3axis, x4, x4axis, x5, x5axis, x6, x6axis, x7, x7axis, x8, x8axis, x9, x9axis);
  } else if (x2axis == 9) {
    return m_AxesO8[x2].GetBinContent(x1, x1axis, x3, x3axis, x4, x4axis, x5, x5axis, x6, x6axis, x7, x7axis, x8, x8axis, x9, x9axis);
  } else if (x3axis == 9) {
    return m_AxesO8[x3].GetBinContent(x1, x1axis, x2, x2axis, x4, x4axis, x5, x5axis, x6, x6axis, x7, x7axis, x8, x8axis, x9, x9axis);
  } else if (x4axis == 9) {
    return m_AxesO8[x4].GetBinContent(x1, x1axis, x2, x2axis, x3, x3axis, x5, x5axis, x6, x6axis, x7, x7axis, x8, x8axis, x9, x9axis);
  } else if (x5axis == 9) {
    return m_AxesO8[x5].GetBinContent(x1, x1axis, x2, x2axis, x3, x3axis, x4, x4axis, x6, x6axis, x7, x7axis, x8, x8axis, x9, x9axis);
  } else if (x6axis == 9) {
    return m_AxesO8[x6].GetBinContent(x1, x1axis, x2, x2axis, x3, x3axis, x4, x4axis, x5, x5axis, x7, x7axis, x8, x8axis, x9, x9axis);
  } else if (x7axis == 9) {
    return m_AxesO8[x7].GetBinContent(x1, x1axis, x2, x2axis, x3, x3axis, x4, x4axis, x5, x5axis, x6, x6axis, x8, x8axis, x9, x9axis);
  } else if (x8axis == 9) {
    return m_AxesO8[x8].GetBinContent(x1, x1axis, x2, x2axis, x3, x3axis, x4, x4axis, x5, x5axis, x6, x6axis, x7, x7axis, x9, x9axis);
  } else if (x9axis == 9) {
    return m_AxesO8[x9].GetBinContent(x1, x1axis, x2, x2axis, x3, x3axis, x4, x4axis, x5, x5axis, x6, x6axis, x7, x7axis, x8, x8axis);
  } else {
    merr<<"Axes are not correctly defined: x1-axis="<<x1axis
        ...
        <<" x8-axis="<<x8axis<<" x9-axis="<<x9axis<<endl;
    return 0;
  }
}
...
float MResponseMatrixO9::GetInterpolated(float x1, float x2, float x3, float x4, 
                                         float x5, float x6, float x7, float x8, 
                                         float x9, bool DoExtrapolate) const
{
  // Return the array-data according to value x1, x2, x3, x4, x5, x6, x7, x8, x9
  ...
  if (m_ValuesCentered == true) { 
    if (m_AxisO9.size() == 2) {
      return m_AxesO8.front().GetInterpolated(x1, x2, x3, x4, x5, x6, x7, x8, DoExtrapolate);
    } else {
      // Get Position:
      int Position = FindBinCentered(m_AxisO9, x9);
      ...
      } else if (Position >= int(m_AxesO8.size()-1)) {
        if (DoExtrapolate == true) {
          Position = int(m_AxesO8.size()-2); // extrapolate above higher edge
          // limits of highest CENTERED bin are m_AxesO8.size()-2 and m_AxesO8.size()-1 !!
        } else {
          return m_AxesO8.back().GetInterpolated(x1, x2, x3, x4, x5, x6, x7, x8, DoExtrapolate);
        }
      }

      // Interpolate:
      return m_AxesO8[Position].GetInterpolated(x1, x2, x3, x4, x5, x6, x7, x8, DoExtrapolate) + (x9 - GetAxisBinCenter(Position))/
        (GetAxisBinCenter(Position+1) - GetAxisBinCenter(Position))*
        (m_AxesO8[Position+1].GetInterpolated(x1, x2, x3, x4, x5, x6, x7, x8, DoExtrapolate) - 
         m_AxesO8[Position].GetInterpolated(x1, x2, x3, x4, x5, x6, x7, x8, DoExtrapolate));
    }
  } else {
    // Get Position:
 		int Position = FindBin(m_AxisO9, x9);
    ...
    // Take care of boundaries:
		...
		} else if (Position >= int(m_AxisO9.size()-1)) {
			if (DoExtrapolate == true) {
				Position = int(m_AxisO9.size()-2); // extrapolate above higher edge
				// limits of highest bin are m_AxisO9.size()-2 and  m_AxisO9.size()-1 !!
			} else {
				return m_AxesO8.back().GetInterpolated(x1, x2, x3, x4, x5, x6, x7, x8, DoExtrapolate);
			}
		}
    ...
    // Interpolate:
    return m_AxesO8[Position].GetInterpolated(x1, x2, x3, x4, x5, x6, x7, x8, DoExtrapolate) + (x9 - m_AxisO9[Position])/
      (m_AxisO9[Position+1] - m_AxisO9[Position])*
      (m_AxesO8[Position+1].GetInterpolated(x1, x2, x3, x4, x5, x6, x7, x8, DoExtrapolate) - 
       m_AxesO8[Position].GetInterpolated(x1, x2, x3, x4, x5, x6, x7, x8, DoExtrapolate));
  }
}
...
float MResponseMatrixO9::Get(float x1, float x2, float x3, float x4, float x5, 
                             float x6, float x7, float x8, float x9) const
{
  ...
  return m_AxesO8[Position].Get(x1, x2, x3, x4, x5, x6, x7, x8);
}
...
float MResponseMatrixO9::GetMaximum() const
{
  // Return the maximum
...
float MResponseMatrixO9::GetMinimum() const
{
  // Return the minimum
...
MResponseMatrixO1 MResponseMatrixO9::GetSumMatrixO1(unsigned int a1) const
{
  // Return the sum of the matrix as function of dimension order
  // Attention: Very inefficient algorithm!

  massert(AreIncreasing(9, a1));
  ...
  if (a1 == 9) {
    for (unsigned int i = 0; i < m_AxesO8.size(); ++i) {
      R.SetBinContent(i, m_AxesO8[i].GetSum());
    }
  } else {
    for (unsigned int i = 0; i < m_AxesO8.size(); ++i) {
      R += m_AxesO8[i].GetSumMatrixO1(a1);
    }
  }
...
MResponseMatrixO2 MResponseMatrixO9::GetSumMatrixO2(unsigned int a1, 
                                                    unsigned int a2) const
{
  // Return the sum of the matrix as function of dimension order
  // Attention: Simple, but very inefficient algorithm!

  massert(AreIncreasing(9, a1, a2));
  ...
  if (a2 == 9) {
    for (unsigned int i = 0; i < m_AxesO8.size(); ++i) {
      R.SetMatrix(i, m_AxesO8[i].GetSumMatrixO1(a1));
    }
  } else {
    for (unsigned int i = 0; i < m_AxesO8.size(); ++i) {
      R += m_AxesO8[i].GetSumMatrixO2(a1, a2);
    }
  }
...
MResponseMatrixO3 MResponseMatrixO9::GetSumMatrixO3(unsigned int a1, 
                                                    unsigned int a2,
                                                    unsigned int a3) const
{
  ...
  massert(AreIncreasing(9, a1, a2, a3));
  ...
  if (a3 == 9) {
    for (unsigned int i = 0; i < m_AxesO8.size(); ++i) {
      R.SetMatrix(i, m_AxesO8[i].GetSumMatrixO2(a1, a2));
    }
  } else {
    for (unsigned int i = 0; i < m_AxesO8.size(); ++i) {
      R += m_AxesO8[i].GetSumMatrixO3(a1, a2, a3);
    }
  }
...
MResponseMatrixO4 MResponseMatrixO9::GetSumMatrixO4(unsigned int a1, 
                                                    unsigned int a2,
                                                    unsigned int a3,
                                                    unsigned int a4) const
{
  ...
  massert(AreIncreasing(9, a1, a2, a3, a4));
  ...
  if (a4 == 9) {
    for (unsigned int i = 0; i < m_AxesO8.size(); ++i) {
      R.SetMatrix(i, m_AxesO8[i].GetSumMatrixO3(a1, a2, a3));
    }
  } else {
    for (unsigned int i = 0; i < m_AxesO8.size(); ++i) {
      R += m_AxesO8[i].GetSumMatrixO4(a1, a2, a3, a4);
    }
  }
...
MResponseMatrixO5 MResponseMatrixO9::GetSumMatrixO5(unsigned int a1, 
                                                    unsigned int a2,
                                                    unsigned int a3,
                                                    unsigned int a4,
                                                    unsigned int a5) const
{
  ...
  massert(AreIncreasing(9, a1, a2, a3, a4, a5));
  ...
  if (a5 == 9) {
    for (unsigned int i = 0; i < m_AxesO8.size(); ++i) {
      R.SetMatrix(i, m_AxesO8[i].GetSumMatrixO4(a1, a2, a3, a4));
    }
  } else {
    for (unsigned int i = 0; i < m_AxesO8.size(); ++i) {
      R += m_AxesO8[i].GetSumMatrixO5(a1, a2, a3, a4, a5);
    }
  }
...
MResponseMatrixO6 MResponseMatrixO9::GetSumMatrixO6(unsigned int a1, 
                                                    unsigned int a2,
                                                    unsigned int a3,
                                                    unsigned int a4,
                                                    unsigned int a5,
                                                    unsigned int a6) const
{
  ...
  massert(AreIncreasing(9, a1, a2, a3, a4, a5, a6));
  ...
  if (a6 == 9) {
    for (unsigned int i = 0; i < m_AxesO8.size(); ++i) {
      R.SetMatrix(i, m_AxesO8[i].GetSumMatrixO5(a1, a2, a3, a4, a5));
    }
  } else {
    for (unsigned int i = 0; i < m_AxesO8.size(); ++i) {
      R += m_AxesO8[i].GetSumMatrixO6(a1, a2, a3, a4, a5, a6);
    }
  }
...
MResponseMatrixO7 MResponseMatrixO9::GetSumMatrixO7(unsigned int a1, 
                                                    unsigned int a2,
                                                    unsigned int a3,
                                                    unsigned int a4,
                                                    unsigned int a5,
                                                    ...
                                                    unsigned int a7) const
{
  ...
  massert(AreIncreasing(9, a1, a2, a3, a4, a5, a6, a7));
  ...
  if (a7 == 9) {
    for (unsigned int i = 0; i < m_AxesO8.size(); ++i) {
      R.SetMatrix(i, m_AxesO8[i].GetSumMatrixO6(a1, a2, a3, a4, a5, a6));
    }
  } else {
    for (unsigned int i = 0; i < m_AxesO8.size(); ++i) {
      R += m_AxesO8[i].GetSumMatrixO7(a1, a2, a3, a4, a5, a6, a7);
    }
  }
...
MResponseMatrixO8 MResponseMatrixO9::GetSumMatrixO8(unsigned int a1, 
                                                    unsigned int a2,
                                                    unsigned int a3,
                                                    unsigned int a4,
                                                    unsigned int a5,
                                                    ...
                                                    unsigned int a8) const
{
  ...
  massert(AreIncreasing(9, a1, a2, a3, a4, a5, a6, a7, a8));
  ...
  if (a8 == 9) {
    for (unsigned int i = 0; i < m_AxesO8.size(); ++i) {
      R.SetMatrix(i, m_AxesO8[i].GetSumMatrixO7(a1, a2, a3, a4, a5, a6, a7));
    }
  } else {
    for (unsigned int i = 0; i < m_AxesO8.size(); ++i) {
      R += m_AxesO8[i].GetSumMatrixO8(a1, a2, a3, a4, a5, a6, a7, a8);
    }
  }
...
MResponseMatrixO9 MResponseMatrixO9::GetSumMatrixO9(unsigned int a1, 
                                                    unsigned int a2, 
                                                    unsigned int a3, 
                                                    unsigned int a4, 
                                                    unsigned int a5, 
                                                    ...
                                                    unsigned int a9) const
{
  ...
  massert(AreIncreasing(9, a1, a2, a3, a4, a5, a6, a7, a8, a9));
...
bool MResponseMatrixO9::ReadSpecific(MFileResponse& Parser, 
                                     const MString& Type, 
                                     const int Version)
{
...
//     }
    ...
    while (Parser.TokenizeLine(T) == true) {
      ...
      if (T.GetTokenAt(0) == "A1") {
        ...
        if ((x1Axis.size()-1)*(x2Axis.size()-1)*(x3Axis.size()-1)*
            (x4Axis.size()-1)*(x5Axis.size()-1)*(x6Axis.size()-1)*
            (x7Axis.size()-1)*(x8Axis.size()-1)*(x9Axis.size()-1) != StreamSize) {
          mout<<"MResponseMatrixO9: Axis sizes and stream size are not in sync!"<<endl;
          Ok = false;
          break;
        }
        SetAxis(x1Axis, x2Axis, x3Axis, x4Axis, x5Axis, x6Axis, x7Axis, x8Axis, x9Axis);
        SetAxisNames(x1Name, x2Name, x3Name, x4Name, x5Name, x6Name, x7Name, x8Name, x9Name);
                    ...
                    for (x3 = 0; x3 < x3_max; ++x3) {
                      for (x2 = 0; x2 < x2_max; ++x2) {
                        for (x1 = 0; x1 < x1_max; ++x1) {
                          if (StreamSize == 0) {
                            mout<<"MResponseMatrixO9: Stream has not the correct length (too small)!"<<endl;
                            StreamOk = false;
                            break;
                          }
                          if (Parser.GetFloat(Data) == true) {
                            SetBinContent(x1, x2, x3, x4, x5, x6, x7, x8, x9, Data);
                            StreamSize--;
                          } else {
                            mout<<"MResponseMatrixO9: Cannot parse a float!"<<endl;
                            StreamOk = false;
                            break;
                          }
                        }
                        ...
                        if (StreamOk == false) {
                          mout<<"MResponseMatrixO9: Stream was not ok!"<<endl;
                          Ok = false;
                          break;
                        }
                      }
    ...
    mout<<"MResponseMatrixO9: Wrong file type: "<<Type<<endl;
    Ok = false;    
  }
...
bool MResponseMatrixO9::Write(MString FileName, bool Stream)
{
            ...
            for (x5 = 0; x5 < x5_max; ++x5) {
              for (x4 = 0; x4 < x4_max; ++x4) {
                for (x3 = 0; x3 < x3_max; ++x3) {
                  for (x2 = 0; x2 < x2_max; ++x2) {
                    for (x1 = 0; x1 < x1_max; ++x1) {
                      File.Write(GetBinContent(x1, x2, x3, x4, x5, x6, x7, x8, x9));
                    }
                  }
                }
              }
  ...
  mdebug<<"File \""<<FileName<<"\" with "
        <<x1_max*x2_max*x3_max*x4_max*x5_max*x6_max*x7_max*x8_max*x9_max
...
void MResponseMatrixO9::Show(float x1, float x2, float x3, float x4, 
                             float x5, float x6, float x7, float x8, 
                             float x9, bool Normalize)
{
  ...
  if (GetNBins() > 0) {
    ...
    if (NAxes == 1) {
      ...
      Hist = new TH1D(m_Name + "_RM9", m_Name + "_RM9", GetAxisBins(axes[0]), Bins);
      Hist->SetStats(true);
      Hist->SetContour(50);
      Hist->SetXTitle(GetAxisName(axes[0]));
      delete [] Bins;
    
      mout<<"Response matrix of order 9 with "<<GetNBins()<<" entries:"<<endl;
      double Norm = 1;
      for (unsigned int i1 = 0; i1 < GetAxisBins(axes[0]); ++i1) {
        ...
        Hist->SetBinContent(i1+1, GetBinContent(i1, axes[0], 
                                                ...
                                                values[6], axes[6], 
                                                values[7], axes[7], 
                                                values[8], axes[8])*Norm);
      }
    
      TCanvas* Canvas = new TCanvas(m_Name + "_RM9C", m_Name + "_RM9C", 0, 0, 600, 600);
      Canvas->cd();
      Hist->Draw();
      Canvas->Update();
      ...
      Hist = new TH2D(m_Name+"_RM9", m_Name+"_RM9", GetAxisBins(axes[0]), xBins, GetAxisBins(axes[1]), yBins);
      Hist->SetStats(true);
      Hist->SetContour(50);
      Hist->SetXTitle(GetAxisName(axes[0]));
      ...
      Hist = new TH3D(m_Name+"_RM9", m_Name+"_RM9", 
    ...
    mout<<"Empty response matrix of order 9"<<endl;
  }
}
...
ostream& operator<<(ostream& os, const MResponseMatrixO9& R)
{
  if (R.GetNBins() > 0) {
    os<<"Response matrix of order 9 with "<<R.GetNBins()<<" entries:"<<endl;
                  ...
                  for (unsigned int i2 = 0; i2 < R.GetAxisBins(2); ++i2) {
                    for (unsigned int i1 = 0; i1 < R.GetAxisBins(1); ++i1) {
                      os<<"x1/x2/x3/x4/x5/x6/x7/x8/x9: "
                        ...
                        <<"  - value : "<<R.GetBinContent(i1, i2, i3, i4, i5, i6, i7, i8, i9)<<endl;
                    }
                  }
                }
              }
    ...
    os<<"Empty response matrix of order 9"<<endl;
  }
  return os;
}
