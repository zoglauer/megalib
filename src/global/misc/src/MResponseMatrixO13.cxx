...
MResponseMatrixO13::MResponseMatrixO13() :
  MResponseMatrixOx()
{
  // default constructor
...
MResponseMatrixO13::MResponseMatrixO13(vector<float> x1Axis, vector<float> x2Axis, 
  ...
  MResponseMatrixOx()
{
  ...
  SetAxis(x1Axis, x2Axis, x3Axis, x4Axis, x5Axis, x6Axis, x7Axis, x8Axis, 
          x9Axis, x10Axis, x11Axis, x12Axis, x13Axis);
}
...
MResponseMatrixO13::MResponseMatrixO13(MString Name, 
...
bool MResponseMatrixO13::operator==(const MResponseMatrixO13& R)
{
  // Two matrixes are identical if they have the same axis:

  if (m_AxisO13 != R.m_AxisO13) {
    return false;
  }

  for (unsigned int i = 0; i < m_AxesO12.size(); ++i) {
    if (!(m_AxesO12[i] == R.m_AxesO12[i])) {
      return false;
    }
  }
...
MResponseMatrixO13& MResponseMatrixO13::operator+=(const MResponseMatrixO13& R)
{
  // Append a matrix to this one

  if (*this == R) {
    for (unsigned int i = 0; i < m_AxesO12.size(); ++i) {
      m_AxesO12[i] += R.m_AxesO12[i];
    }
  } else {
...
MResponseMatrixO13& MResponseMatrixO13::operator+=(const float& Value)
{
  // Append a matrix to this one
...
void MResponseMatrixO13::SetAxis(vector<float> x1Axis, vector<float> x2Axis, 
                                 vector<float> x3Axis, vector<float> x4Axis, 
                                 vector<float> x5Axis, vector<float> x6Axis, 
                                 vector<float> x7Axis, vector<float> x8Axis, 
                                 vector<float> x9Axis, vector<float> x10Axis, 
                                 ...
                                 vector<float> x13Axis)
{
  ...
  m_AxesO12.resize(m_AxisO13.size()-1);
  for (unsigned int b = 0; b < m_AxisO13.size()-1; ++b) {
    m_AxesO12[b].SetAxis(x1Axis, x2Axis, x3Axis, x4Axis, x5Axis, x6Axis, 
                         x7Axis, x8Axis, x9Axis, x10Axis, x11Axis, x12Axis);
    m_AxesO12[b].SetName(m_Name);
  }
}
...
void MResponseMatrixO13::SetAxisNames(MString x1Name, MString x2Name, 
                                      ...
                                      MString x13Name)
{
  ...
  m_NameAxisO13 = x13Name;
  for (unsigned int i = 0; i < m_AxesO12.size(); ++i) {
    m_AxesO12[i].SetAxisNames(x1Name, x2Name, x3Name, x4Name, x5Name, x6Name, 
...
MString MResponseMatrixO13::GetAxisName(unsigned int order) const
{
  // Return the name of the axis

  massert(order >= 1 && order <= 13);

  if (order == 13) {
    return m_NameAxisO13;
  } else {
    massert(m_AxisO13.size() > 0);
    return m_AxesO12[0].GetAxisName(order);
  }
}
...
void MResponseMatrixO13::SetBinContent(unsigned int x1, unsigned int x2, 
                                       ...
                                       unsigned int x13, float Value)
{
  // Set the content of the bin

  massert(x13 < m_AxisO13.size()-1);
  
  m_AxesO12[x13].SetBinContent(x1, x2, x3, x4, x5, x6, x7, x8, x9, x10, x11, x12, Value);
}
...
void MResponseMatrixO13::Add(float x1, float x2, float x3, float x4, 
                             float x5, float x6, float x7, float x8, 
                             float x9, float x10, float x11, float x12, 
                             float x13, float Value)
{
  /// Add a value to the bin closest to x, y, z

  // Get Position:
  int Position = -1; 
  for (unsigned int i = 0; i < m_AxisO13.size(); ++i) {
    if (m_AxisO13[i] > x13) {
      break;
    } 
    Position = (int) i;
  ...
  if (Position >= (int) m_AxisO13.size()-1) {
    mdebug<<"Overflow in "
  ...
  m_AxesO12[Position].Add(x1, x2, x3, x4, x5, x6, x7, x8, x9, x10, x11, x12, Value);
}
...
void MResponseMatrixO13::SetMatrix(unsigned int b, MResponseMatrixO12 R)
{
  // Set a whole sub matrix

  massert(b < m_AxesO12.size());
  m_AxesO12[b] = R;
}
...
unsigned long MResponseMatrixO13::GetNBins() const
{
  // Return the number of bins

  if (m_AxisO13.size() < 2) return 0;
  unsigned long Bins = m_AxisO13.size()-1;
  return Bins*m_AxesO12[0].GetNBins();
}
...
unsigned int MResponseMatrixO13::GetAxisBins(unsigned int order) const
{
  // Return the number of bins for the main axis O13
  ...
  if (order == 13) {
    ...
    return m_AxisO13.size()-1;
  } else {
    massert(m_AxisO13.size() > 0);
    return m_AxesO12[0].GetAxisBins(order);
  }
}
...
vector<float> MResponseMatrixO13::GetAxis(unsigned int order) const
{
  ...
  if (order == 13) {
    ...
    massert(GetNBins() > 0);
    return m_AxesO12[0].GetAxis(order);
  }
}
...
float MResponseMatrixO13::GetAxisContent(unsigned int b, unsigned int order) const
{
  // Return the axis content at bin b
  ...
  if (order == 13) {
    massert(b < m_AxisO13.size());
    return m_AxisO13[b];
  } else {
    massert(GetNBins() > 0);
    return m_AxesO12[0].GetAxisContent(b, order);
  }
}
...
float MResponseMatrixO13::GetAxisBinCenter(unsigned int b, unsigned int order) const
{
  ...
  if (order == 13) {
    massert(b < m_AxisO13.size()-1);
    return 0.5*(m_AxisO13[b]+m_AxisO13[b+1]);
  } else {
    massert(GetNBins() > 0);
    return m_AxesO12[0].GetAxisBinCenter(b, order);
  }
}
...
unsigned int MResponseMatrixO13::GetAxisBin(float x, unsigned int order) const
{
  // Return the center of axis bin b
  ...
  if (order == 13) {
    ...
    if (Position >= m_AxisO13.size()-1) {
      Position = c_Outside;
    }
    return Position;
  } else {
    massert(GetNBins() > 0);
    return m_AxesO12[0].GetAxisBin(x, order);
  }
}
...
float MResponseMatrixO13::GetAxisMinimum(unsigned int order) const
{
  ...
  if (order == 13) {
    ...
    massert(GetNBins() > 0);
    return m_AxesO12[0].GetAxisMinimum(order);
  }
}
...
float MResponseMatrixO13::GetAxisMaximum(unsigned int order) const
{
  ...
  if (order == 13) {
    massert(m_AxisO13.size() > 0);
    return m_AxisO13[m_AxisO13.size()-1];
  } else {
    massert(GetNBins() > 0);
    return m_AxesO12[0].GetAxisMaximum(order);
  }
}
...
float MResponseMatrixO13::GetAxisLowEdge(unsigned int b, unsigned int order) const
{
  ...
  if (order == 13) {
    ...
    massert(GetNBins() > 0);
    return m_AxesO12[0].GetAxisLowEdge(b, order);
  }
}
...
float MResponseMatrixO13::GetAxisHighEdge(unsigned int b, unsigned int order) const
{
  ...
  if (order == 13) {
    massert(b < m_AxisO13.size()-1);
    return m_AxisO13[b+1];
  } else {
    massert(GetNBins() > 0);
    return m_AxesO12[0].GetAxisHighEdge(b, order);
  } 
}
...
float MResponseMatrixO13::GetBinContent(unsigned int x1, unsigned int x2, 
                                        unsigned int x3, unsigned int x4, 
                                        unsigned int x5, unsigned int x6, 
                                        unsigned int x7, unsigned int x8, 
                                        unsigned int x9, unsigned int x10, 
                                        ...
                                        unsigned int x13) const
{
  ...
  return m_AxesO12[x13].GetBinContent(x1, x2, x3, x4, x5, x6, x7, x8, x9, x10, x11, x12);
}
...
float MResponseMatrixO13::GetBinArea(unsigned int x1, unsigned int x2, 
                                     unsigned int x3, unsigned int x4, 
                                     unsigned int x5, unsigned int x6, 
                                     unsigned int x7, unsigned int x8, 
                                     unsigned int x9, unsigned int x10, 
                                     ...
                                     unsigned int x13) const
{
  ...
  return (m_AxisO13[x13+1]-m_AxisO13[x13])*m_AxesO12[x13].GetBinArea(x1, x2, x3, x4, x5, x6, x7, x8, x9, x10, x11, x12);
}
...
float MResponseMatrixO13::GetBinContent(unsigned int x1, unsigned int x1axis, 
                                        ...
                                        unsigned int x13, unsigned int x13axis) const
{
  ...
  if (x1axis == 13) {
    return m_AxesO12[x1].GetBinContent(x2, x2axis, x3, x3axis, x4, x4axis, x5, x5axis, x6, x6axis, x7, x7axis, x8, x8axis, 
                                      x9, x9axis, x10, x10axis, x11, x11axis, x12, x12axis, x13, x13axis);
  } else if (x2axis == 13) {
    return m_AxesO12[x2].GetBinContent(x1, x1axis, x3, x3axis, x4, x4axis, x5, x5axis, x6, x6axis, x7, x7axis, x8, x8axis, 
                                      x9, x9axis, x10, x10axis, x11, x11axis, x12, x12axis, x13, x13axis);
  } else if (x3axis == 13) {
    return m_AxesO12[x3].GetBinContent(x1, x1axis, x2, x2axis, x4, x4axis, x5, x5axis, x6, x6axis, x7, x7axis, x8, x8axis, 
                                      x9, x9axis, x10, x10axis, x11, x11axis, x12, x12axis, x13, x13axis);
  } else if (x4axis == 13) {
    return m_AxesO12[x4].GetBinContent(x1, x1axis, x2, x2axis, x3, x3axis, x5, x5axis, x6, x6axis, x7, x7axis, x8, x8axis, 
                                      x9, x9axis, x10, x10axis, x11, x11axis, x12, x12axis, x13, x13axis);
  } else if (x5axis == 13) {
    return m_AxesO12[x5].GetBinContent(x1, x1axis, x2, x2axis, x3, x3axis, x4, x4axis, x6, x6axis, x7, x7axis, x8, x8axis, 
                                      x9, x9axis, x10, x10axis, x11, x11axis, x12, x12axis, x13, x13axis);
  } else if (x6axis == 13) {
    return m_AxesO12[x6].GetBinContent(x1, x1axis, x2, x2axis, x3, x3axis, x4, x4axis, x5, x5axis, x7, x7axis, x8, x8axis, 
                                      x9, x9axis, x10, x10axis, x11, x11axis, x12, x12axis, x13, x13axis);
  } else if (x7axis == 13) {
    return m_AxesO12[x7].GetBinContent(x1, x1axis, x2, x2axis, x3, x3axis, x4, x4axis, x5, x5axis, x6, x6axis, x8, x8axis, 
                                      x9, x9axis, x10, x10axis, x11, x11axis, x12, x12axis, x13, x13axis);
  } else if (x8axis == 13) {
    return m_AxesO12[x8].GetBinContent(x1, x1axis, x2, x2axis, x3, x3axis, x4, x4axis, x5, x5axis, x6, x6axis, x7, x7axis, 
                                      x9, x9axis, x10, x10axis, x11, x11axis, x12, x12axis, x13, x13axis);
  } else if (x9axis == 13) {
    return m_AxesO12[x9].GetBinContent(x1, x1axis, x2, x2axis, x3, x3axis, x4, x4axis, x5, x5axis, x6, x6axis, x7, x7axis, 
                                      x8, x8axis, x10, x10axis, x11, x11axis, x12, x12axis, x13, x13axis);
  } else if (x10axis == 13) {
    return m_AxesO12[x10].GetBinContent(x1, x1axis, x2, x2axis, x3, x3axis, x4, x4axis, x5, x5axis, x6, x6axis, x7, x7axis, 
                                       x8, x8axis, x9, x9axis, x11, x11axis, x12, x12axis, x13, x13axis);
  } else if (x11axis == 13) {
    return m_AxesO12[x11].GetBinContent(x1, x1axis, x2, x2axis, x3, x3axis, x4, x4axis, x5, x5axis, x6, x6axis, x7, x7axis, 
                                       x8, x8axis, x9, x9axis, x10, x10axis, x12, x12axis, x13, x13axis);
  } else if (x12axis == 13) {
    return m_AxesO12[x12].GetBinContent(x1, x1axis, x2, x2axis, x3, x3axis, x4, x4axis, x5, x5axis, x6, x6axis, x7, x7axis, 
                                        x8, x8axis, x9, x9axis, x10, x10axis, x11, x11axis, x13, x13axis);
  } else if (x13axis == 13) {
    return m_AxesO12[x13].GetBinContent(x1, x1axis, x2, x2axis, x3, x3axis, x4, x4axis, x5, x5axis, x6, x6axis, x7, x7axis, 
                                        x8, x8axis, x9, x9axis, x10, x10axis, x11, x11axis, x12, x12axis);
  } else {
    merr<<"Axes are not correctly defined: x1-axis="<<x1axis
        ...
        <<" x12-axis="<<x12axis<<" x13-axis="<<x13axis<<endl;
    return 0;
  }
}
...
float MResponseMatrixO13::GetInterpolated(float x1, float x2, float x3, float x4, 
                                          float x5, float x6, float x7, float x8, 
                                          float x9, float x10, float x11, float x12, 
                                          float x13, bool DoExtrapolate) const
{
  // Return the array-data according to value x1, x2, x3, x4, x5, x6, x7, x8, x9, x10, x11, x12, x13
  ...
  if (m_ValuesCentered == true) { 
    if (m_AxisO13.size() == 2) {
      return m_AxesO12.front().GetInterpolated(x1, x2, x3, x4, x5, x6, x7, x8, x9, x10, x11, x12, DoExtrapolate);
    } else {
      // Get Position:
      int Position = FindBinCentered(m_AxisO13, x13);
      ...
      } else if (Position >= int(m_AxesO12.size()-1)) {
        if (DoExtrapolate == true) {
          Position = int(m_AxesO12.size()-2); // extrapolate above higher edge
          // limits of highest CENTERED bin are m_AxesO12.size()-2 and m_AxesO12.size()-1 !!
        } else {
          return m_AxesO12.back().GetInterpolated(x1, x2, x3, x4, x5, x6, x7, x8, x9, x10, x11, x12, DoExtrapolate);
        }
      }

      // Interpolate:
      return m_AxesO12[Position].GetInterpolated(x1, x2, x3, x4, x5, x6, x7, x8, x9, x10, x11, x12, DoExtrapolate) + (x13 - GetAxisBinCenter(Position))/
        (GetAxisBinCenter(Position+1) - GetAxisBinCenter(Position))*
        (m_AxesO12[Position+1].GetInterpolated(x1, x2, x3, x4, x5, x6, x7, x8, x9, x10, x11, x12, DoExtrapolate) - 
         m_AxesO12[Position].GetInterpolated(x1, x2, x3, x4, x5, x6, x7, x8, x9, x10, x11, x12, DoExtrapolate));
    }
  } else {
    // Get Position:
    int Position = FindBin(m_AxisO13, x13);
    ...
    } else if (Position >= int(m_AxisO13.size()-1)) {
      if (DoExtrapolate == true) {
        Position = int(m_AxisO13.size()-2); // extrapolate above higher edge
        // limits of highest bin are m_AxisO13.size()-2 and  m_AxisO13.size()-1 !!
      } else {
        return m_AxesO12.back().GetInterpolated(x1, x2, x3, x4, x5, x6, x7, x8, x9, x10, x11, x12, DoExtrapolate);
      }
    }
    ...
    // Interpolate:
    return m_AxesO12[Position].GetInterpolated(x1, x2, x3, x4, x5, x6, x7, x8, x9, x10, x11, x12, DoExtrapolate) + (x13 - m_AxisO13[Position])/
      (m_AxisO13[Position+1] - m_AxisO13[Position])*
      (m_AxesO12[Position+1].GetInterpolated(x1, x2, x3, x4, x5, x6, x7, x8, x9, x10, x11, x12, DoExtrapolate) - 
       m_AxesO12[Position].GetInterpolated(x1, x2, x3, x4, x5, x6, x7, x8, x9, x10, x11, x12, DoExtrapolate));
  }
}
...
float MResponseMatrixO13::Get(float x1, float x2, float x3, float x4, float x5, 
                              float x6, float x7, float x8, float x9, float x10, 
                              float x11, float x12, float x13) const
{
  ...
  return m_AxesO12[Position].Get(x1, x2, x3, x4, x5, x6, x7, x8, x9, x10, x11, x12);
}
...
float MResponseMatrixO13::GetMaximum() const
{
  // Return the maximum
...
float MResponseMatrixO13::GetMinimum() const
{
  // Return the minimum
...
double MResponseMatrixO13::GetSum() const
{
  // Return the sum of all bins:

  double Sum = 0;
  for (unsigned int i = 0; i < m_AxesO12.size(); ++i) {
    Sum += m_AxesO12[i].GetSum();
  }  
...
MResponseMatrixO1 MResponseMatrixO13::GetSumMatrixO1(unsigned int a1) const
{
  // Return the sum of the matrix as function of dimension order
  // Attention: Very inefficient algorithm!

  massert(AreIncreasing(13, a1));
  ...
  if (a1 == 13) {
    for (unsigned int i = 0; i < m_AxesO12.size(); ++i) {
      R.SetBinContent(i, m_AxesO12[i].GetSum());
    }
  } else {
    for (unsigned int i = 0; i < m_AxesO12.size(); ++i) {
      R += m_AxesO12[i].GetSumMatrixO1(a1);
    }
  }
...
MResponseMatrixO2 MResponseMatrixO13::GetSumMatrixO2(unsigned int a1, 
                                                    unsigned int a2) const
{
  // Return the sum of the matrix as function of dimension order
  // Attention: Simple, but very inefficient algorithm!

  massert(AreIncreasing(13, a1, a2));
  ...
  if (a2 == 13) {
    for (unsigned int i = 0; i < m_AxesO12.size(); ++i) {
      R.SetMatrix(i, m_AxesO12[i].GetSumMatrixO1(a1));
    }
  } else {
    for (unsigned int i = 0; i < m_AxesO12.size(); ++i) {
      R += m_AxesO12[i].GetSumMatrixO2(a1, a2);
    }
  }
...
MResponseMatrixO3 MResponseMatrixO13::GetSumMatrixO3(unsigned int a1, 
                                                    unsigned int a2,
                                                    unsigned int a3) const
{
  ...
  massert(AreIncreasing(13, a1, a2, a3));
  ...
  if (a3 == 13) {
    for (unsigned int i = 0; i < m_AxesO12.size(); ++i) {
      R.SetMatrix(i, m_AxesO12[i].GetSumMatrixO2(a1, a2));
    }
  } else {
    for (unsigned int i = 0; i < m_AxesO12.size(); ++i) {
      R += m_AxesO12[i].GetSumMatrixO3(a1, a2, a3);
    }
  }
...
MResponseMatrixO4 MResponseMatrixO13::GetSumMatrixO4(unsigned int a1, 
                                                    unsigned int a2,
                                                    unsigned int a3,
                                                    unsigned int a4) const
{
  ...
  massert(AreIncreasing(13, a1, a2, a3, a4));
  ...
  if (a4 == 13) {
    for (unsigned int i = 0; i < m_AxesO12.size(); ++i) {
      R.SetMatrix(i, m_AxesO12[i].GetSumMatrixO3(a1, a2, a3));
    }
  } else {
    for (unsigned int i = 0; i < m_AxesO12.size(); ++i) {
      R += m_AxesO12[i].GetSumMatrixO4(a1, a2, a3, a4);
    }
  }
...
MResponseMatrixO5 MResponseMatrixO13::GetSumMatrixO5(unsigned int a1, 
                                                    unsigned int a2,
                                                    unsigned int a3,
                                                    unsigned int a4,
                                                    unsigned int a5) const
{
  ...
  massert(AreIncreasing(13, a1, a2, a3, a4, a5));
  ...
  if (a5 == 13) {
    for (unsigned int i = 0; i < m_AxesO12.size(); ++i) {
      R.SetMatrix(i, m_AxesO12[i].GetSumMatrixO4(a1, a2, a3, a4));
    }
  } else {
    for (unsigned int i = 0; i < m_AxesO12.size(); ++i) {
      R += m_AxesO12[i].GetSumMatrixO5(a1, a2, a3, a4, a5);
    }
  }
...
MResponseMatrixO6 MResponseMatrixO13::GetSumMatrixO6(unsigned int a1, 
                                                    unsigned int a2,
                                                    unsigned int a3,
                                                    unsigned int a4,
                                                    unsigned int a5,
                                                    unsigned int a6) const
{
  ...
  massert(AreIncreasing(13, a1, a2, a3, a4, a5, a6));
  ...
  if (a6 == 13) {
    for (unsigned int i = 0; i < m_AxesO12.size(); ++i) {
      R.SetMatrix(i, m_AxesO12[i].GetSumMatrixO5(a1, a2, a3, a4, a5));
    }
  } else {
    for (unsigned int i = 0; i < m_AxesO12.size(); ++i) {
      R += m_AxesO12[i].GetSumMatrixO6(a1, a2, a3, a4, a5, a6);
    }
  }
...
MResponseMatrixO7 MResponseMatrixO13::GetSumMatrixO7(unsigned int a1, 
                                                    unsigned int a2,
                                                    unsigned int a3,
                                                    unsigned int a4,
                                                    unsigned int a5,
                                                    ...
                                                    unsigned int a7) const
{
  ...
  massert(AreIncreasing(13, a1, a2, a3, a4, a5, a6, a7));
  ...
  if (a7 == 13) {
    for (unsigned int i = 0; i < m_AxesO12.size(); ++i) {
      R.SetMatrix(i, m_AxesO12[i].GetSumMatrixO6(a1, a2, a3, a4, a5, a6));
    }
  } else {
    for (unsigned int i = 0; i < m_AxesO12.size(); ++i) {
      R += m_AxesO12[i].GetSumMatrixO7(a1, a2, a3, a4, a5, a6, a7);
    }
  }
...
MResponseMatrixO8 MResponseMatrixO13::GetSumMatrixO8(unsigned int a1, 
                                                     unsigned int a2,
                                                     unsigned int a3,
                                                     unsigned int a4,
                                                     unsigned int a5,
                                                     ...
                                                     unsigned int a8) const
{
  ...
  massert(AreIncreasing(13, a1, a2, a3, a4, a5, a6, a7, a8));
  ...
  if (a8 == 13) {
    for (unsigned int i = 0; i < m_AxesO12.size(); ++i) {
      R.SetMatrix(i, m_AxesO12[i].GetSumMatrixO7(a1, a2, a3, a4, a5, a6, a7));
    }
  } else {
    for (unsigned int i = 0; i < m_AxesO12.size(); ++i) {
      R += m_AxesO12[i].GetSumMatrixO8(a1, a2, a3, a4, a5, a6, a7, a8);
    }
  }
...
MResponseMatrixO9 MResponseMatrixO13::GetSumMatrixO9(unsigned int a1, 
                                                     unsigned int a2,
                                                     unsigned int a3,
                                                     unsigned int a4,
                                                     unsigned int a5,
                                                     ...
                                                     unsigned int a9) const
{
  ...
  massert(AreIncreasing(13, a1, a2, a3, a4, a5, a6, a7, a8, a9));
  ...
  if (a9 == 13) {
    for (unsigned int i = 0; i < m_AxesO12.size(); ++i) {
      R.SetMatrix(i, m_AxesO12[i].GetSumMatrixO8(a1, a2, a3, a4, a5, a6, a7, a8));
    }
  } else {
    for (unsigned int i = 0; i < m_AxesO12.size(); ++i) {
      R += m_AxesO12[i].GetSumMatrixO9(a1, a2, a3, a4, a5, a6, a7, a8, a9);
    }
  }
...
MResponseMatrixO10 MResponseMatrixO13::GetSumMatrixO10(unsigned int a1, 
                                                       unsigned int a2,
                                                       unsigned int a3,
                                                       unsigned int a4,
                                                       unsigned int a5,
                                                       ...
                                                       unsigned int a10) const
{
  ...
  massert(AreIncreasing(13, a1, a2, a3, a4, a5, a6, a7, a8, a9, a10));
  ...
  if (a10 == 13) {
    for (unsigned int i = 0; i < m_AxesO12.size(); ++i) {
      R.SetMatrix(i, m_AxesO12[i].GetSumMatrixO9(a1, a2, a3, a4, a5, a6, a7, a8, a9));
    }
  } else {
    for (unsigned int i = 0; i < m_AxesO12.size(); ++i) {
      R += m_AxesO12[i].GetSumMatrixO10(a1, a2, a3, a4, a5, a6, a7, a8, a9, a10);
    }
  }
...
MResponseMatrixO11 MResponseMatrixO13::GetSumMatrixO11(unsigned int a1, 
                                                       unsigned int a2,
                                                       unsigned int a3,
                                                       unsigned int a4,
                                                       unsigned int a5,
                                                       ...
                                                       unsigned int a11) const
{
  ...
  massert(AreIncreasing(13, a1, a2, a3, a4, a5, a6, a7, a8, a9, a10, a11));
  ...
  if (a11 == 13) {
    for (unsigned int i = 0; i < m_AxesO12.size(); ++i) {
      R.SetMatrix(i, m_AxesO12[i].GetSumMatrixO10(a1, a2, a3, a4, a5, a6, a7, a8, a9, a10));
    }
  } else {
    for (unsigned int i = 0; i < m_AxesO12.size(); ++i) {
      R += m_AxesO12[i].GetSumMatrixO11(a1, a2, a3, a4, a5, a6, a7, a8, a9, a10, a11);
    }
  }
...
MResponseMatrixO12 MResponseMatrixO13::GetSumMatrixO12(unsigned int a1, 
                                                       unsigned int a2,
                                                       unsigned int a3,
                                                       unsigned int a4,
                                                       unsigned int a5,
                                                       ...
                                                       unsigned int a12) const
{
  ...
  massert(AreIncreasing(13, a1, a2, a3, a4, a5, a6, a7, a8, a9, a10, a11, a12));
  ...
  if (a12 == 13) {
    for (unsigned int i = 0; i < m_AxesO12.size(); ++i) {
      R.SetMatrix(i, m_AxesO12[i].GetSumMatrixO11(a1, a2, a3, a4, a5, a6, a7, a8, a9, a10, a11));
    }
  } else {
    for (unsigned int i = 0; i < m_AxesO12.size(); ++i) {
      R += m_AxesO12[i].GetSumMatrixO12(a1, a2, a3, a4, a5, a6, a7, a8, a9, a10, a11, a12);
    }
  }
...
MResponseMatrixO13 MResponseMatrixO13::GetSumMatrixO13(unsigned int a1, 
                                                       unsigned int a2, 
                                                       unsigned int a3, 
                                                       unsigned int a4, 
                                                       unsigned int a5, 
                                                       ...
                                                       unsigned int a13) const
{
  ...
  massert(AreIncreasing(13, a1, a2, a3, a4, a5, a6, a7, a8, a9, a10, a11, a12, a13));
                                     ...
                                     const int Version)
{
  ...
  if (Type == "ResponseMatrixO13") {
    merr<<"MResponseMatrixO13: Non-stream reading is not supported!"<<endl;
    /*
    while (Parser.TokenizeLine(T, true) == true) {
      ...
      if (T.GetTokenAt(0) == "R2") {
        if (T.GetNTokens() == 4) {
          ...
          mout<<"MResponseMatrixO13: Wrong number of arguments for token R2!"<<endl;
          Ok = false;
          break;
        }
      }
    ...
    */
    Ok = false;
  } else if (Type == "ResponseMatrixO13Stream") {
    vector<float> x1Axis;
    vector<float> x2Axis;
    vector<float> x3Axis;
    vector<float> x4Axis;
    ...
    while (Parser.TokenizeLine(T, true) == true) {
      ...
      if (T.GetTokenAt(0) == "A1") {
        ...
        if ((x1Axis.size()-1)*(x2Axis.size()-1)*(x3Axis.size()-1)*
            ...
            (x7Axis.size()-1)*(x8Axis.size()-1)*(x9Axis.size()-1)*
            (x10Axis.size()-1)*(x11Axis.size()-1)*(x12Axis.size()-1)*
            (x13Axis.size()-1) != StreamSize) {
          mout<<"MResponseMatrixO13: Axis sizes and stream size are not in sync!"<<endl;
          Ok = false;
          break;
        }
        SetAxis(x1Axis, x2Axis, x3Axis, x4Axis, x5Axis, x6Axis, x7Axis, x8Axis, x9Axis, x10Axis, x11Axis, x12Axis, x13Axis);
        SetAxisNames(x1Name, x2Name, x3Name, x4Name, x5Name, x6Name, x7Name, x8Name, x9Name, x10Name, x11Name, x12Name, x13Name);
                            ...
                            for (x3 = 0; x3 < x3_max; ++x3) {
                              for (x2 = 0; x2 < x2_max; ++x2) {
                                for (x1 = 0; x1 < x1_max; ++x1) {
                                  if (StreamSize == 0) {
                                    mout<<"MResponseMatrixO13: Stream has not the correct length (too small)!"<<endl;
                                    StreamOk = false;
                                    break;
                                  }
                                  if (Parser.GetFloat(Data) == true) {
                                    SetBinContent(x1, x2, x3, x4, x5, x6, x7, x8, x9, x10, x11, x12, x13, Data);
                                    StreamSize--;
                                  } else {
                                    mout<<"MResponseMatrixO13: Cannot parse a float!"<<endl;
                                    StreamOk = false;
                                    break;
                                  }
                                }
                                ...
                                if (StreamOk == false) {
                                  mout<<"MResponseMatrixO13: Stream was not ok!"<<endl;
                                  Ok = false;
                                  break;
                                }
                              }
    ...
    mout<<"MResponseMatrixO13: Wrong file type: "<<Type<<endl;
    Ok = false;    
  }
...
bool MResponseMatrixO13::Write(MString FileName, bool Stream)
{
  ...
  if (Stream == false) {
    merr<<"MResponseMatrixO13: Non-stream writing is not supported!"<<endl;
    /*
    s<<"Type ResponseMatrixO13"<<endl;
    for (x13 = 0; x13 < x13_max; ++x13) {
      for (x12 = 0; x12 < x12_max; ++x12) {
        for (x11 = 0; x11 < x11_max; ++x11) {
          for (x10 = 0; x10 < x10_max; ++x10) {
                          ...
                          for (x2 = 0; x2 < x2_max; ++x2) {
                            for (x1 = 0; x1 < x1_max; ++x1) {
                              s<<"R13 "
                                ...
                                <<GetAxisContent(x13, 13)<<" "
                                <<" "<<GetBinContent(x1, x2, x3, x4, x5, x6, x7, x8, x9, x10, x11, x12, x13)<<endl;
                              File.Write(s);
                            }
                          }
                        }
    ...
    */
    ...
    s<<"Type ResponseMatrixO13Stream"<<endl;
    // Write x1-axis
    s<<"A1 ";
    File.Write(s);
    for (x1 = 0; x1 <= x1_max; ++x1) {
                    ...
                    for (x5 = 0; x5 < x5_max; ++x5) {
                      for (x4 = 0; x4 < x4_max; ++x4) {
                        for (x3 = 0; x3 < x3_max; ++x3) {
                          for (x2 = 0; x2 < x2_max; ++x2) {
                            for (x1 = 0; x1 < x1_max; ++x1) {
                              File.Write(GetBinContent(x1, x2, x3, x4, x5, x6, x7, x8, x9, x10, x11, x12, x13));
                            }
                          }
                        }
                      }
  ...
  mdebug<<"File \""<<FileName<<"\" with "
        <<x1_max*x2_max*x3_max*x4_max*x5_max*x6_max*x7_max*x8_max*x9_max*x10_max*x11_max*x12_max*x13_max
...
void MResponseMatrixO13::Show(float x1, float x2, float x3, float x4, 
                              float x5, float x6, float x7, float x8, 
                              float x9, float x10, float x11, 
                              float x12, float x13, bool Normalize)
{
  // Create a 3d ROOT histogram:
  
  vector<unsigned int> axes;
  vector<unsigned int> values;
  vector<bool> found(13, false);
  ...
  if (GetNBins() > 0) {
    ...
    if (NAxes == 1) {
      ...
      Hist = new TH1D(m_Name + "_RM13", m_Name + "_RM13", GetAxisBins(axes[0]), Bins);
      Hist->SetStats(true);
      Hist->SetContour(50);
      Hist->SetXTitle(GetAxisName(axes[0]));
      delete [] Bins;
    
      mout<<"Response matrix of order 13 with "<<GetNBins()<<" entries:"<<endl;
      double Norm = 1;
      for (unsigned int i1 = 0; i1 < GetAxisBins(axes[0]); ++i1) {
        ...
        Hist->SetBinContent(i1+1, GetBinContent(i1, axes[0], 
                                                ...
                                                values[10], axes[10], 
                                                values[11], axes[11], 
                                                values[12], axes[12])*Norm);
      }
    
      TCanvas* Canvas = new TCanvas(m_Name + "_RM13C", m_Name + "_RM13C", 0, 0, 600, 600);
      Canvas->cd();
      Hist->Draw();
      Canvas->Update();
      ...
      Hist = new TH2D(m_Name+"_RM13", m_Name+"_RM13", GetAxisBins(axes[0]), xBins, GetAxisBins(axes[1]), yBins);
      Hist->SetStats(true);
      Hist->SetContour(50);
      Hist->SetXTitle(GetAxisName(axes[0]));
      ...
      Hist = new TH3D(m_Name+"_RM13", m_Name+"_RM13", 
    ...
    mout<<"Empty response matrix of order 13"<<endl;
  }
}
...
{
  if (R.GetNBins() > 0) {
    os<<"Response matrix of order 13 with "<<R.GetNBins()<<" entries:"<<endl;
                        ...
                        for (unsigned int i3 = 0; i3 < R.GetAxisBins(3); ++i3) {
                          for (unsigned int i2 = 0; i2 < R.GetAxisBins(2); ++i2) {
                            for (unsigned int i1 = 0; i1 < R.GetAxisBins(1); ++i1) {
                              os<<"x1/x2/x3/x4/x5/x6/x7/x8/x9/x10/x11/x12/x13: "
                                ...
                                <<"  - value : "<<R.GetBinContent(i1, i2, i3, i4, i5, i6, i7, i8, i9, i10, i11, i12, i13)<<endl;
                            }
                          }
                        }
                      }
    ...
    os<<"Empty response matrix of order 13"<<endl;
  }
  return os;
}


// MResponseMatrixO13.cxx: the end...
////////////////////////////////////////////////////////////////////////////////
