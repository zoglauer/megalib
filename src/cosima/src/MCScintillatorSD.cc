...
{ 
  ...
  if (Energy <= 0.0) {
    mout<<"MCScintillatorSD: Info: Energy loss map resulted in zero or negative energy: "<<m_EnergyLoss.Eval(Position.getX()/cm, Position.getY()/cm, Position.getZ()/cm)<<":"<<Position<<endl;    
    return false;
  }
  ...
  // Now create a hit:
  MCScintillatorHit* Hit = new MCScintillatorHit();
  Hit->SetEnergy(Energy);
  Hit->SetADCCounts(Energy/keV);
  Hit->SetPosition(Position);
  Hit->SetDetectorName(Hist->GetVolume(Hist->GetHistoryDepth()-1)->GetName());
