...
G4bool MCDriftChamberSD::PostProcessHits(const G4Step* Step)
{
  ...
  if (Energy <= 0.0) {
    mout<<"MCDriftChamberSD: Info: Energy loss map resulted in zero or negative energy: "<<m_EnergyLoss.Eval(Position.getX()/cm, Position.getY()/cm, Position.getZ()/cm)<<":"<<Position<<endl;    
    return false;
  }
