    ...
    iraw(false)
{
  ...
  SetLowEnergyLimit(2*theElectron->GetPDGMass());
}
                                         ...
                                         G4double, G4double)
{
  ...
  const G4Element* anElement  = SelectRandomAtom(couple, theGamma, GammaEnergy);
//                                         , aDynamicGamma->GetLogKineticEnergy() );
  // Atomic number
  const G4int Z       = lrint(anElement->GetZ());
