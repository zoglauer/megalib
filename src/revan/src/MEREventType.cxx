                             ...
                             vector<MString> DetectorList)
{
  ...
  m_SearchPhoto = SearchPhoto;
  m_SearchCompton = SearchCompton;
...
//  } else {
  for (unsigned int n = 0; n < DetectorList.size(); ++n) {//Ignored if no detector used for tracking
    bool Found = false;
  ...
  if (m_DetectorList.size() == 0 && (m_SearchMIPs || m_SearchPairs) ) {
...
*/
  ...
  Timer.Start();
  if (StartTracking && !Identified && m_SearchMIPs) {
