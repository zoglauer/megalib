 ...
 * Default constructor
 */
SimFilter::SimFilter() : m_Interrupt(false)
{
 ...
 * Parse the command line
 */
bool SimFilter::ParseCommandLine(int argc, char** argv)
{
  ostringstream Usage;
  Usage<<endl;
  Usage<<"  Usage: SimFilter <options>"<<endl;
 ...
 * Analyze the event, return true if it has to be writen to file
 */
bool SimFilter::AnalyzeEvent(MSimEvent* Event)
{
  // Add your code here
  // Return true if the event should be written to file

  // Need at least 2 interactions
  if (Event->GetNIAs() < 2) return false;

  // The second interactions needs to be a pair in the cross-strip detectr
  if (Event->GetIAAt(1)->GetProcess() != "PAIR" || Event->GetIAAt(1)->GetDetector() != 1) return false;
...
int main(int argc, char** argv)
{
  ...
  TApplication SimFilterApp("SimFilterApp", 0, 0);
